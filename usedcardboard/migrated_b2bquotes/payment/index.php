...
//echo "nyn<pre>"; print_r($_REQUEST); echo "</pre>"; exit();
...
if(isset($_REQUEST['quote_id'])){ 
	...
	$getOrderId = db_query("SELECT id AS OrderId FROM b2becommerce_order_item WHERE quote_id ='".$quote_id."' AND session_id = '".$sessionId."'", db());
	$rowOrderId = array_shift($getOrderId);

	$getOrderedProd = db_query("SELECT * FROM b2becommerce_order_item_details WHERE order_item_id = '".$rowOrderId['OrderId']."'", db());
	...
	$getCurrData = db_query("SELECT * FROM b2becommerce_order_item WHERE session_id = '".$sessionId."' and quote_id = '" . $quote_id ."'", db() );
	...
	if($_REQUEST['rdoPickUp'] == 'Customer Pickup'){
		$qryUpdt = "UPDATE b2becommerce_order_item SET pickup_type = '". str_replace("'", "\'" ,$_REQUEST['rdoPickUp']) ."', billing_firstname = '". str_replace("'", "\'" ,$_REQUEST['txtbillingaddFNm']) ."', billing_lastname = '". str_replace("'", "\'" ,$_REQUEST['txtbillingaddLNm']) ."', billing_company = '". str_replace("'", "\'" ,$_REQUEST['txtbillingAddCompny']) ."', billing_add1 = '". str_replace("'", "\'" ,$billingAdd['billingAdd1']) ."', billing_add2 = '". str_replace("'", "\'" ,$billingAdd['billingAdd2']) ."', billing_city = '". str_replace("'", "\'" ,$billingAdd['billingAddCity']) ."', billing_state = '". str_replace("'", "\'" ,$billingAdd['billingAddState'])."', billing_zip = '". str_replace("'", "\'" ,$billingAdd['billingAddZip'])."', billing_email = '". str_replace("'", "\'" ,$billingAdd['billingAddEmail']) ."', billing_phone = '". str_replace("'", "\'" ,$billingAdd['billingAddPhn']) ."' WHERE session_id = '".$sessionId."' and product_loopboxid = '" . $ProductLoopId ."'";
	}else{

		$qryUpdt = "UPDATE b2becommerce_order_item SET billing_add1 = '". str_replace("'", "\'" ,$billingAdd['billingAdd1'])."', billing_add2 = '". str_replace("'", "\'" ,$billingAdd['billingAdd2'])."', billing_city = '". str_replace("'", "\'" ,$billingAdd['billingAddCity'])."', billing_state = '". str_replace("'", "\'" ,$billingAdd['billingAddState']) ."', billing_zip = '".str_replace("'", "\'" ,$billingAdd['billingAddZip'])."', billing_email = '".str_replace("'", "\'" ,$billingAdd['billingAddEmail'])."', billing_phone = '".str_replace("'", "\'" ,$billingAdd['billingAddPhn']) ."' WHERE session_id = '".$sessionId."' and product_loopboxid = '" . $ProductLoopId ."'";
	}
	...
	$po_no	= "";
	$getCompId = db_query("SELECT ID, companyID, poNumber FROM quote WHERE ID = ".$quote_id, db_b2b());	
	$rowCompID = array_shift($getCompId);
	if(!empty($rowCompID['companyID'])){
		$credit_term_companyID = $rowCompID['companyID'];
		$po_no = $rowCompID['poNumber'];
		
		$getLoopId = db_query("SELECT loopid, parent_comp_id FROM companyInfo WHERE ID = ".$credit_term_companyID, db_b2b());
		$rowLoopId = array_shift($getLoopId);
		$warehouse_id = $rowLoopId['loopid'];
		$parent_child_compid = $rowLoopId["parent_comp_id"]; 
			...
			<div class="new_container no-top-padding">
				...
				<div class="content-div content-padding ">
					<div class="left_form">
						...
						<form id="post_data" name="post_data" action="../authorizenet_frm_action.php" method="post" >
									...
									<fieldset class="content-box" >
										<div class="radio-wrapper content-box__row" data-same-billing-address="" onclick="rdoStatusRow(1)">
											<div class="radio__input">
												<?php  
													//echo " credit_application_net_term - " . $credit_application_net_term;
													$credit_term_str = ""; $credit_cc_str = " checked "; $response_transId = "credit_card";
													if ($credit_application_net_term != "") { 
														$credit_term_str = " checked ";
														$response_transId = "credit_term";
														$credit_cc_str = "";
													} 
												?>
												
												<input class="input-radio" data-backup="credit_term_false" type="radio" value="same" "<?php echo $credit_cc_str;?>" name="rdoCc_or_credit_term" id="rdoCreditCard" onchange="rdoStatus()" >
										...
										<div class="radio-wrapper content-box__row" data-same-billing-address="" onclick="rdoStatusRow(2)">
											<div class="radio__input">
												...
												<input class="input-radio" data-backup="credit_term_true"  "<?php echo $credit_term_str;?>" 
										...
										<div data-address-field="phone" data-autocomplete-field-container="true" class="radio-wrapper content-box__row">
										  ...
										  <div class="field__input-wrapper"><label class="radio__label content-box__emphasis cc_or_term" style="text-align: left;">Purchase Order Number</label>
											...
											value="<?php echo $po_no;?>" class="field__input" size="30" type="text" name="txtPO" id="txtPO">
										  </div>
										  <br>
										</div>
										<div class="bottomBorder"></div>
										...
										<div id="div_credit_term">
											...
											$cc_fees = 0;
											if ($total > 0){
												$cc_fees = str_replace(",", "" ,number_format($total,2)) * 0.03;
											}
											...
											<!-- <form id="post_data" name="post_data" action="https://b2b.usedcardboardboxes.com/authorizenet_frm_action.php" method="post" > -->
												<input type="hidden" name="response_transId" id="response_transId" value="<?php echo $response_transId; ?>" />
												<input type="hidden" name="response_amt" id="response_amt" value="<?php echo round($finalTotal);?>" />
												...
												<input type="hidden" name="total_amt" id="total_amt" value="<?php echo number_format($finalTotal, 2);?>" />
												<input type="hidden" id="totalcnt_withccfee" value="<?php echo "$". number_format($finalTotal,2);?>">
												<input type="hidden" id="totalcnt_withoutccfee" value="<?php echo "$". number_format($total,2); ?>">
				...
				<div class="innerdiv_2">
					...
					<div class="sidebar-sept"></div>
						...
						<table class="sidebar-table" id="paymenttbl">
							...
							if(!empty($arrProdDt)){ 
								...
								?>
									<tr>
										...
										<td id="totolProQnt" align="center"><?php echo number_format(str_replace(",", "" , $arrProdDtV['productQnt']),0) ; ?></td>
										<td align="right">$<?php echo number_format(str_replace(",", "" ,$arrProdDtV['productQntprice']),2); ?></td>
										<td align="right">$<?php echo number_format(trim($arrProdDtV['productTotal']), 2); ?></td>
									</tr>
								<?php 
								$total = $total + $arrProdDtV['productTotal']; 
								}
							...
							<?php 
							$getQuoteShipping = db_query("SELECT free_shipping FROM quote WHERE ID = ".$quote_id, db_b2b());
							...
							<tr>
								<td></td>
								<td></td>
								<td style="font-weight: 500;"  align="right" class="payment-due__price">Total</td>
								<td align="right" ><!-- USD&nbsp;&nbsp; --><span class="payment-due__price"><?php 
									$finalTotal = ($total + $cc_fees);
							...
							<tr id="ccfee_tr">
								...
								<td align="right" >$<?php echo number_format($cc_fees,2) ; ?><br><span id="totalcnt"><?php echo "$". number_format($finalTotal,2); ?></span></td>
							</tr>
						</table>
						</div>
						
						<?php  
							$getQuotePickup = db_query("SELECT via FROM quote WHERE ID = ".$quote_id, db_b2b());
						...
						<div style="padding-top: 60px;">
							<ol class="name-values" style="width: 100%;">
									<li>                    
										...
										<span id="about">
											<?php if(!empty($rowCurrData['contact_firstname']) || !empty($rowCurrData['contact_lastname']) ){ 
												echo $rowCurrData['contact_firstname'] ." ".$rowCurrData['contact_lastname']; 
											} ?><?php if(!empty($rowCurrData['contact_company'])){ 
									...
									<li>
										...
										<span id="Span1">
											<?php if($quotePickupType == 'Pickup' ) { ?>
												Customer Pickup  
											<?php } else { ?>
												<?php if(!empty($rowCurrData['shipping_add1']) ){ 
													echo $rowCurrData['shipping_add1']; 
												} ?><?php if( !empty($rowCurrData['shipping_add2']) ){ 
													echo ", ".$rowCurrData['shipping_add2']; 
												} ?><?php if( !empty($rowCurrData['shipping_city']) ){ 
													echo ", ".$rowCurrData['shipping_city']; 
												} ?><?php if( !empty($rowCurrData['shipping_state']) ){ 
													echo ", ".$rowCurrData['shipping_state']; 
												} ?><?php if( !empty($rowCurrData['shipping_zip']) ){ 
...
<?php }else{ ?>
	...
	$sessionId = session_id();
	$getCurrData = db_query("SELECT b2becommerce_order_item.*, b2becommerce_order_item_details.order_item_id, b2becommerce_order_item_details.product_name_id, b2becommerce_order_item_details.product_name, b2becommerce_order_item_details.product_qty, b2becommerce_order_item_details.product_unitprice, b2becommerce_order_item_details.product_total FROM b2becommerce_order_item INNER JOIN b2becommerce_order_item_details ON b2becommerce_order_item_details.order_item_id = b2becommerce_order_item.id WHERE session_id = '".$sessionId."' and product_loopboxid = '" . $ProductLoopId ."'", db() );
	 ...
	 */
		...
		function rdoStatus(){
			...
			if(rdoVal == 'same'){
				...
				var inputval = 'credit_card';
				document.getElementById("ccfee_tr").style.display = "contents";
				document.getElementById("totalcnt").innerHTML = document.getElementById("totalcnt_withccfee").value;
								...
								<div class="section__content">
									<fieldset class="content-box" >
										...
										<div id="div_credit_term">
											...
											<?php 
											if(!empty($rowCurrData['product_total']) && !empty($rowCurrData['quote_total']) ){
												$productTotal = str_replace(",", "", $rowCurrData['product_total']);
												$productTotal = str_replace("$", "", $productTotal);
												$total = $productTotal + str_replace(",", "", trim($rowCurrData['quote_total']));
											}elseif(!empty($rowCurrData['product_total']) ){	
												$productTotal = str_replace(",", "", $rowCurrData['product_total']);
												$productTotal = str_replace("$", "", $productTotal);
												$total = $productTotal;
											}else{
											...
											$cc_fees = 0;
											if ($totalTemp > 0){
												$cc_fees = str_replace(",", "" ,number_format($totalTemp,2)) * 0.03;
											}	
											$totalTemp =  str_replace("$", "", $total);
											...
											<!-- <form id="post_data" name="post_data" action="https://b2b.usedcardboardboxes.com/authorizenet_frm_action.php" method="post" > -->
												...
												<input type="hidden" name="response_amt" id="response_amt" value="<?php echo round($total);?>" />
												<input type="hidden" name="productIdloop" id="productIdloop" value="<?php echo $ProductLoopId;?>" />
												<input type="hidden" name="total_amt" id="total_amt" value="<?php echo $total;?>" />
												<input type="hidden" name="hdnLastInsertId" value="<?php echo $_REQUEST['hdnLastInsertId']; ?>">
												<input type="hidden" id="hdAvailability" name="hdAvailability" value="<?php echo $orderData['hdAvailability'];?>">
												<input type="hidden" id="totalcnt_withccfee" value="<?php echo "$". number_format($total,2);?>">
												<input type="hidden" id="totalcnt_withoutccfee" value="<?php echo "$". number_format($totalTemp,2); ?>">
												<button type="submit" class="button_slide slide_right" data-testid="order-button">Place order</button>
				...
				<div class="innerdiv_2">
					...
					<div class="sidebar-sept"></div>
						...
						<table class="sidebar-table" id="paymenttbl">
							...
							<tr>
								<td><?php echo $rowCurrData['product_name'] ; ?></td>
								<td id="totolProQnt" align="center"><?php echo number_format(str_replace(",", "" , $rowCurrData['product_qty']),0) ; ?></td>
								<td align="right">$<?php echo number_format(str_replace(",", "" ,$rowCurrData['product_unitprice']),2) ; ?></td>
								<td align="right">$<?php echo number_format(str_replace("$", "" ,str_replace(",", "" ,trim($rowCurrData['product_total']))),2);?></td>
							</tr>

							<?php if ($customer_pickup == "no") { ?>
								<tr>
									<td>
										<?php if(!empty($rowCurrData['quote_name']) ){ 
											echo $rowCurrData['quote_name'];
										} ?>
									</td>
									<td align="center">
										<?php if(empty($rowCurrData['quote_qty']) ){ 
											echo '0'; 
										}else { 
									...
									<td align="right">
										<?php if(empty($rowCurrData['quote_unit_price']) ){ 
											echo '$0.00'; 
										}else { 
									...
									<td align="right">
										<?php if(empty($rowCurrData['quote_total']) ){ 
											echo '$0.00'; 
										}else { 
								...
								$cc_fees = 0;
								if ($totalTemp > 0){
									$cc_fees = number_format(str_replace(",", "" ,number_format($totalTemp,2)) * 0.03,2);
								}	
								?>
							<tr><td colspan="4"><div class="sidebar-sept-intable"></div></td></tr>
							<tr>
							...
							<tr id="ccfee_tr">
								<td></td>
								<td colspan="2" style="font-weight: 500;"  align="right">Convenience Fee (3%)<br>Total if Paid by Credit Card</td>
								<td align="right" >$<?php echo $cc_fees ; ?><br><span id="totalcnt"><?php echo "$". number_format($total,2); ?></span></td>
							</tr>
						</table>
						</div>
