...
$idTitle = ""; $browserTitle = ""; $pgTitle = ""; $fullUnitPr = 0; $halfUnitPr = 0; $qrtrUnitPr = 0; $palletUnitPr = 0;
...
function highlightRow(){
	...
	function calculatedistance()
	{
		...
		if(counter > 5){
			...
			xmlhttp.open("POST","calculate_distance.php?zip=" + zip + "&id=<?php echo$_REQUEST['id'];?>", true);
			xmlhttp.send();
		}
	}
...
db();
$getSessionDt = db_query("SELECT id FROM b2becommerce_tracking WHERE session_id = '".$sessionId."' and product_loopboxid = '".$id."'");
...
if($rowSessionDt["id"] != ""){
	...
	db_query("INSERT INTO b2becommerce_tracking(session_id, product_loopboxid, machine_ip, user_master_id, record_date) VALUES('".$sessionId."', '".$id."', '".$machineIP."', '".$userId."', '".$recordDt."')");
}
			...
			<div class="content-div content-padding">
				<div class="left_qty">
					<table class="qty-table-top">
						<tr>
							...
							if ($row["lead_time"] <= 1){
								$leadTimeCal = "Next Day";	
							}else{
								$leadTimeCal = $row["lead_time"] . " Days";	
							}
				        ...
				        <tr>
							...
							<td>
								<form action="" method="post">
									<input class="field__input" type="text" name="distzip" id="distzip" value="<?php echo $loczip?>" size="10" maxlength="7" placeholder="Zip code">
					...
					<div class="qty-div">
						<table class="qty-table" id="productInfo">
							...
							<tr id="productinfo_1" class="highlightrow" onclick="javascript: productinfo(1);">
								<td align="left">
									<input id="radio1" type="radio" name="radio" value="1" <?php echo $data_product_name1;?> <?php echo($row["ship_ltl"]=="0")? ' checked ' : ' ';?> onclick="javascript: productinfo(1);">
									<label for="radio1"><span><span></span></span>
									<label id="productQntype1">Full</label></label>

								</td>
								<td>
									<span id="productQnt1"><?php echo number_format((float)trim($row["boxes_per_trailer"]));?>
									</span>
									
								</td>
								<td>
									$<span id="productQntprice1"><?php echo number_format($fullUnitPr,2);?>
									</span>
									
								</td>
								<td>
									<?php $priceUnit = number_format($fullUnitPr,2); ?>
									$<span id="productTotal1"><?php echo number_format(($row["boxes_per_trailer"] * $priceUnit),2);?></span>
								</td>
							</tr>
							<?php  
							if($row["ship_ltl"] == 0){	
							...
							?>
								<tr id="productinfo_2" onclick="javascript: productinfo(2);" >
									<td align="left">
										<input id="radio2" type="radio" name="radio" value="2" <?php echo $data_product_name2;?> onclick="javascript: productinfo(2);" >
										<label for="radio2"><span><span></span></span>
										<label id="productQntype2">Half</label></label>
									</td>
									<td>
										<span id="productQnt2"><?php echo number_format($row["boxes_per_trailer"]/2);?></span>
									</td>
									<td>
										$<span id="productQntprice2"><?php echo number_format($halfUnitPr, 2);?></span>
									</td>
									<td>
										<?php $halfpriceUnit = number_format($halfUnitPr, 2); ?>
										$<span id="productTotal2"><?php echo number_format((float)(((float)$row["boxes_per_trailer"]/2)*(float)$halfpriceUnit), 2);?></span>
									</td>
								</tr>
								<tr id="productinfo_3" onclick="javascript: productinfo(3);" >
									<td align="left">
										<input id="radio3" type="radio" name="radio" value="3" <?php echo $data_product_name3;?> onclick="javascript: productinfo(3);">
										<label for="radio3"><span><span></span></span><label id="productQntype3">Quarter</label></label>

									</td>
									<td>
										<?php $qrtQtyCal = number_format( ceil( $row["boxes_per_trailer"]/$row["bpallet_qty"]/4 ) * $row["bpallet_qty"] );?>
										<span id="productQnt3"><?php echo $qrtQtyCal; ?></span>
									</td>
									<td>
										$<span id="productQntprice3"><?php echo number_format($qrtrUnitPr, 2);?></span>
									</td>
									<td>
										<?php $qrtrpriceUnit = number_format($qrtrUnitPr, 2); ?>
										$<span id="productTotal3"><?php echo number_format(((float)$qrtQtyCal*(float)$qrtrpriceUnit), 2);?></span>
									</td>
								</tr>
								<tr id="productinfo_4" onclick="javascript: productinfo(4);" >
									<td align="left">
										<input id="radio4" type="radio" name="radio" value="4" <?php echo $data_product_name4;?> onclick="javascript: productinfo(4);">
										<label for="radio4"><span><span></span></span><label id="productQntype4">1 Pallet</label></label>
									</td>
									<td>
										<span id="productQnt4"><?php echo number_format($row["bpallet_qty"]);?></span>
									</td>
									<td>
										$<span id="productQntprice4"><?php echo number_format($palletUnitPr, 2);?></span>
									</td>
									<td>
										<?php 
										$palletunitPr = number_format($palletUnitPr, 2);
										?>											
										$<span id="productTotal4"><?php echo number_format(($row["bpallet_qty"] * $palletunitPr), 2); ?></span>
									</td>
								</tr>
							<?php 
							}
					...
					<div class="btn-div" id="order-div">
					  <form action="contact.php" method="post" id="orderfrm">
						...
						<input type="text" id="hdAvailability" name="hdAvailability" value="<?php echo$estimated_next_load;?>">
						<input type="text" id="hdLeadTime" name="hdLeadTime" value="<?php echo$leadTimeCal;?>">
				...
				<div class="right-products">
					...
					<div class="productinfo-main product-box-shadow">
						<h2 class="prod-title"><?php echo $idTitle . ": ". $boxId; ?></h2>
								...
								<div class="row">
									...
									<div class="main-img-slider">
										...
										<?php /* if (@getimagesize($imgpath_internal.$row["bpic_5"])){?>
											<a data-fancybox="gallery" href="<?php echo$imgpath.$row["bpic_5"]?>" data-width="2048" data-height="1365">
												<img src="<?php echo$imgpath.$row["bpic_5"]?>" class="img-fluid">
											</a>
										<?php } */?>
									</div>
