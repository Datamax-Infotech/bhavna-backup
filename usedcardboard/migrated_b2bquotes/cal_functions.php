...
//
function get_geolocation_google(string $address): array|bool
{
...
function get_geolocation(string $ip): bool
{
...
function find_distance_ip(int $id, string $ipadd): float
{
	...
	$location = json_decode(strval($loc), true);
 ...
 * @return mixed[]
 */
function get_latlog(string $zip): array
{
...
function find_distance(string $id, string $zip): int
{
	$qry = "Select * FROM loop_boxes WHERE id = '" . $id  . "'";
	db();
	$res = db_query($qry);
 ...
 * @return string
 */
function uber_freight_data(int $proid, array $orderData): string
{
	...
	$dropoff_nm = trim($orderData['shippingaddFNm'] . " " . $orderData['shippingaddLNm']);
	...
	if ($donotprocess == "no") {
		...
		for ($tmpcnt = 1; $tmpcnt <= 1; $tmpcnt++) {
			...
			$uber_qote_id = 0;
			db();
			$dt_view_res_data = db_query("Select max(id) as maxunqid from b2becommerce_order_item");
			while ($myrowsel_b2b = array_shift($dt_view_res_data)) {
			...
			$quote_req = array(
				'quote_id' => 'UCB_B2B_id' . $uber_qote_id, 'customer_id' => 'USEDCARDBOARDBOXES',
		...
		if ($ifany_err == "") {
			$avg_quote_amount_tot = number_format($quote_amount_tot * 1.1, 2);
		}
	}
