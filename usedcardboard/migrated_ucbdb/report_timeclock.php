		...
		<table cellSpacing="1" cellPadding="1" width="800" border="0">
			...
			?>
				<tr vAlign="center">
					...
					<td bgColor="#e4e4e4" class="style3" style="height: 22px;" align=right>
						<font face="Arial, Helvetica, sans-serif" color="#333333" size="1">
							<?php echo $row["T_I"]; ?>
						</font>
					</td>
					...
					<td bgColor="#e4e4e4" style="height: 22px;" class="style3" align=center>
						<font face="Arial, Helvetica, sans-serif" color="#333333" size="1">
							<a href="report_timeclock.php?worker=<?php echo $_REQUEST["worker"]; ?>&action=run&edit=true&id=<?php echo encrypt_url($row["id"]); ?>&start_date=<?php echo $_REQUEST["start_date"]; ?>&end_date=<?php echo $_REQUEST["end_date"]; ?>">Edit</a>
						</font>
					</td>
					<td bgColor="#e4e4e4" style="height: 22px;" class="style3" align=left>
						<font size=1><?php if ($row["time_in_old"] != "0000-00-00 00:00:00") echo $row["time_in_old"]; ?> <?php if ($row["time_out_old"] != "0000-00-00 00:00:00") echo $row["time_out_old"]; ?> <?php echo $row["notes"]; ?></font>
					</td>
				</tr>
			<?php
			}
...
if ($_REQUEST["edit"] == "true") {
	$query = "SELECT * FROM loop_timeclock WHERE id = " . decrypt_url($_REQUEST["id"]);
	$res = db_query($query);
	while ($row = array_shift($res)) {
?>
		<form name="rptSearch2" action="report_timeclock.php" method="GET">
			...
			<table>
				...
				<tr>
					<td bgColor="#e4e4e4"><select name="worker">
							<option>Please Select</option>
							<?php
...
if ($_REQUEST["action"] == "update") {
	$sql3 = "UPDATE loop_timeclock SET time_in = '" . $_REQUEST["new_time_in"] . "', time_out = '" . $_REQUEST["new_time_out"] . "', time_in_old = '" . $_REQUEST["time_in_old"] . "', time_out_old = '" . $_REQUEST["time_out_old"] . "', worker_id = '" . $_REQUEST["worker"] . "', notes = '" . $_REQUEST["notes"] . "' WHERE id = " . decrypt_url($_REQUEST["timeclockid"]);
	$result3 = db_query($sql3);
