...
$sql_table = ""; $sql_query = ""; $sql_table_warehouse = "";
...
if ($searchcrit == "") {
	$flag = "all";
	//$sql = "SELECT * FROM orders_active_export inner join $sql_table on $sql_table.orders_id = orders_active_export.orders_id ";
	//$sqlcount = "select count(*) as reccount from orders_active_export inner join $sql_table on $sql_table.orders_id = orders_active_export.orders_id ";
...
//echo "<BR>SQL: $sql<BR>";
	...
	if (empty($result)) {
		if (!headers_sent()){    //If headers not sent yet... then do php redirect
			header('Location: http://b2c.usedcardboardboxes.com/shipments_shipped.php?posting=yes&'); exit;
		}
		else
		{
