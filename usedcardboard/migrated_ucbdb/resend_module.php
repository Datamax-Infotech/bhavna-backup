...
?>
        <P>
                <font face="arial" size="2">Are you sure you want to resend the <?php echo $_GET['module_name']; ?> module?
                        <br><br>
...
<?php
if ($_GET['start_date'] == 'action') {
        if (decrypt_url($_GET['warehouse_id']) == '99') {
                $sps_qry = "SELECT * FROM orders_sps WHERE orders_id = " . decrypt_url($_GET['orders_id']);
                ...
                while ($sps_qry_result_array = array_shift($sps_qry_result)) {
                        if ($sps_qry_result_rows > 0) {
                                ...
                                $sql3 = "INSERT INTO ucbdb_crm (orders_id, comm_type, message, message_date, employee) VALUES ( '" . decrypt_url($_GET['orders_id']) . "','" . $comm_type . "','" . $output . "','" . $today . "','" . $_COOKIE['userinitials'] . "')";
                                // echo "<BR>SQL: $sql3<BR>";
        ...
        $cancel_ord_qry = db_query("Update orders_active_export set setignore = 1 where id = " . decrypt_url($_GET['id']));
        $customers_telephone = "";
        $comment = "";
        ...
        $ordersql = "SELECT * FROM orders WHERE orders_id=" . decrypt_url($_GET['orders_id']);
        $order_result = db_query($ordersql);
        ...
        $zip_query = db_query("SELECT W.warehouse_id, name FROM warehouse W INNER JOIN zipcodes Z ON W.warehouse_id=Z.warehouse_id WHERE Z.zip='" . substr($delivery_postcode, 0, 3) . "'");
        $zip_row = array_shift($zip_query);
        ...
        //echo $mod_qry;
        $mod_qry_result = db_query($mod_qry);
