...
<head>
	...
	<script>
		...
		function cancel_labels_process(orders_id){
			if (window.confirm('Do you really want to cancel the label?'))
			{
				window.location = "cancel_order.php?orders_id="+orders_id+"&process=action";
			}
			else
			{
				die();
    ...
    function fun_badadd_copy(order_id)
	{ 
		...
		xmlhttp.onreadystatechange=function()
		{
		  if (xmlhttp.readyState==4 && xmlhttp.status==200)
		  {
			...
			document.getElementById("fed_add_old").innerHTML = xmlhttp.responseText;
		  }
		}
		xmlhttp.open("POST","badadd_copy_data.php?orders_id="+order_id,true);			
		xmlhttp.send();
	}	
	...
	function order_cancel_labels(orders_id, orid){
		if (window.confirm('Do you really want to cancel the label?'))
		{
			//window.location = "cancel_order_label.php?orders_id="+orders_id+"&orid="+orid+"&cancel_flg=yes";
			if (window.XMLHttpRequest)
			{
				xmlhttp=new XMLHttpRequest();
			}
					...
					if(xmlhttp.responseText=="yes")
						{
							alert('Label will get cancelled in DASH, but NOT in FedEx Ship Manager. If label is already printed, you must \n 1. Also Delete it in FedEx Ship Manager AND \n 2. Tell the local Manager to destroy the label and not send out the kit for it. \n 3. Fedex Status is marked Ignore.')
								location.reload(); 
								//window.location = "cancel_order_label.php?orders_id="+orders_id+"&cancel_flg=yes";
			...
			/*if (window.confirm('Label has been cancelled in DASH, but NOT in FedEx Ship Manager. If label is already printed, you must 1. Also Delete it in FedEx Ship Manager AND 2. Tell the local Manager to destroy the label and not send out the kit for it.')){
				window.location = "cancel_order_label.php?orders_id="+orders_id+"&cancel_flg=yes";
			}
			else{
				die();
...
----------------------------------------------------------------*/
	...
	echo "	<TR>";
// echo "<TD><DIV CLASS='TBL_COL_HDR'>View</DIV></TD>"; 
...
<!--			<TD CLASS='<?php  echo $shade; ?>'>
				...
				<?php  if ($allowview == "yes") { ?>
					<!--<a href="<?php  echo $thispage; ?>?id=<?php  echo $id; ?>&proc=View&searchcrit=<?php  echo $searchcrit; ?>&page=<?php  if ($page > 0) { $newpage = $page - 1; echo $newpage; } ?><?php  echo $pagevars; ?>"> 
					View <img src="images/magnifyingglass.gif" alt="" width="16" height="16" border="0"></a>-->
...
<!--				</DIV>
			...
			<TD CLASS='<?php  echo $shade; ?>'>
				<a href="<?php  echo $thispage; ?>?id=<?php  echo $id; ?>&proc=View&searchcrit=<?php  echo $searchcrit; ?>&page=<?php  if ($page > 0) { $newpage = $page - 1; echo $newpage; } ?><?php  echo $pagevars; ?>"><?php  echo $orders_id; ?></a> 
			</TD>
			<TD CLASS='<?php  echo $shade; ?>'>
			...
			<TD CLASS='<?php  echo $shade; ?>'><?php  $order_date = date("F j, Y", strtotime($myrowsel["date_purchased"])); echo $order_date; ?></td>
...
<?php  if ($customers_postcode == $billing_postcode) { ?> <!-------- TO CHECK BILLING ZIP FOR FRAUD ---------> 
			<TD CLASS='<?php  echo $shade; ?>'>
<?php  } else { ?>
<TD bgcolor=red>
<?php  } ?>
...
//echo " <a href=\"javascript: history.go(-1)\">Back</a><br><br>";
 //echo "<br><a href=\"orders.php?posting=yes&searchcrit=" .$searchcrit. "&submit=B1&page=" .$page. "\">Back</a><br><br>";
?>
<br>
...
if ($proc == "View") {
$ubox_order = 0;
//echo "<DIV CLASS='PAGE_STATUS'>Viewing Order</DIV>";
...
if ($myrowsel["data_encrypt"] == 0 ){
  ...
  $cc_type = ""; $cc_owner = ""; $cc_expires = ""; $cc_number = "";
  if ($myrowsel['cc_type'] <> ""){	
...
<td valign="top">
	...
	<table cellSpacing="1" cellPadding="1" border="0" style="width: 300px">
		<tr align="middle">
			...
			SHIPPING DETAILS&nbsp;<a href='#' onclick="editshipping(<?php  echo $orders_id; ?>)">Edit</a></font>
			</td>
		</tr>
		<tr bgColor="#e4e4e4">
		...
		<tr bgColor="#e4e4e4">
			...
			<td align="left" height="13" style="width: 235px" class="style1"><?php  $order_date = date("l, jS F Y", strtotime($date_purchased)); echo $order_date; ?></td>
		</tr>
		<tr bgColor="#e4e4e4">
			<td style="width: 100px; height: 13px;" class="style1">Order Time</td>
			<td align="left" style="width: 235px; height: 13px;" class="style1"><?php  $order_time = date("g:i:s a", strtotime($date_purchased)); echo $order_time; ?></td>
		</tr>
		<tr bgColor="#e4e4e4">
		...
		<tr bgColor="#e4e4e4">
			...
			<td align="left" height="19" style="width: 235px" class="style1">
				<?php  if ($customers_telephone != "") { echo "<img src='images/phone_img.png' width='12px' height='12px'>&nbsp; <a href='tel:$customers_telephone'>" . $customers_telephone . "</a>";} ?>
			</td>
		</tr>
		<tr bgColor="#e4e4e4">
		...
		<tr bgColor="#e4e4e4">
			<td height="13" style="width: 100px" class="style1">Bad Address</font></td>
		...
		<tr bgColor="#e4e4e4">
			<td height="13" style="width: 100px" class="style1">UPS Notes</font></td>
		...
		<table cellSpacing="1" cellPadding="1" border="0" style="width: 300px">
			...
			<tr bgColor="#e4e4e4">
				<td align="left" style="height: 13px;" class="style1" colspan="2">
					<input type="button" name="btnsend_badadd_eml" id="btnsend_badadd_eml" value="Send Email to Customer" onclick="fun_badadd_send_email(<?php  echo $orders_id;?>)"/>		
...
<table cellSpacing="1" cellPadding="1" border="0" style="width: 300px">
	...
	$auth_code_query = db_query("select * from auth_net where type IS NULL AND orders_id = '" . (int)$orders_id . "'");
		  ...
		  ?>		
			<td align="left" height="10" style="width: 177px" class="style1"><?php  if ($auth_code_res["trans_id"] != '') { echo $auth_code_res["trans_id"]; $auth_trans_id = $auth_code_res["trans_id"]; } else { echo "No Id Available"; }  ?></td>
			<?php  } ?>
		</tr>
	</table>
	<?php  } else  { ?>
...
<?php  if ($cancel == "No") { ?>
	...
	?>
	<?php  /*
	$orders_boxbucks_query = db_query("select code, discount, type FROM coupons WHERE id='" . $coupon_id . "'");
		if (tep_db_num_rows($orders_boxbucks_query) > 0) {
		  while ($orders_boxbucks = array_shift($orders_boxbucks_query)) {
		  */
	?>		
	<!--		<tr bgColor="#e4e4e4">
			<td style="width: 310px; height: 13px;" class="style1" >Box Bucks:</td>
			<td style="width: 310px; height: 13px;" class="style1"><?php  echo $orders_boxbucks["code"]; ?></td>
			<td style="width: 127px; height: 13px;" class="style1"><?php  echo $orders_boxbucks["discount"]; ?></td>
			</tr> -->
	<?php  /*
			}	
		}
		...
		<tr bgColor="#e4e4e4">
			<td colspan="3" class="style1">
				<form action="send_orders.php" method="post">
					...
					<input type="hidden" name="ordersid" value="<?php  echo $orders_id; ?>">
...
<?php  }?>
		...
		<table cellSpacing="1" cellPadding="1" border="0" style="width: 300px">
			...
			<tr bgColor="#e4e4e4">
				<td height="13" style="width: 95px" class="style1">Referrer</font></td>
		...
		<?php 
			...
			<table cellSpacing="1" cellPadding="1" border="0" style="width: 300px">
				...
				<tr bgColor="#e4e4e4">
					<td height="13" style="width: 95px" class="style1">Shipping Address</font></td>
...
<!--	<tr bgColor="#e4e4e4">
		<td height="13" style="width: 189px" class="style1">Resend Receipt</td>
		<td align="left" height="13" style="width: 189px" class="style1">Send</td>
	</tr>
	<tr bgColor="#e4e4e4">
		<td height="13" style="width: 189px" class="style1">Resend Tracking Info</td>
				<td align="left" height="13" style="width: 189px" class="style1">Send</td>	</tr>
-->	
<?php  if ($cancel == "Yes") { ?>
	<tr bgColor="#e4e4e4">
...
<!--	
	<tr bgColor="#e4e4e4">
		<td class="style1">Cancel Order w/ Restocking Fee</td>
		<td align="left" class="style1" style="height: 13px; width: 189px;" class="style1">Process</td>
	</tr>
-->	
	</table>
<br>
...
else 
{
	...
	?>
	<option value="<?php  echo $myresissue1["assigned_to"]; ?>"><?php  echo $myresissue1["assigned_to"]; ?></option>
	<?php  
	} 
	?>
	<option></option>
...
?>
	...
	<tr bgColor="#e4e4e4">
		...
		<input type="submit" value="UnMark as Order Issue"></td>
...
<?php  } ?>
	...
	<tr bgColor="#e4e4e4">
		<td height="10" colspan="2" "style="width: 189px" class="style1">
		<input type="submit" value="Mark as Order Issue"></td>
...
<td colspan="3">
	...
	<div id="display_order_database_import" >
		<table cellSpacing="1" cellPadding="1" border="0" style="width: 500px">
			...
			<tr bgColor="#e4e4e4">
				<td height="13" style="width: 100px" class="style1">WAREHOUSE</font></td>
			...
			while ($wrow = array_shift($resw)) {
				...
				if ($wrow["tablename"] != "") {
					$sql_orders_active_table = "SELECT * FROM " . $wrow["tablename"] . " WHERE orders_id = " . $_GET["id"];
					...
					while ($row_active = array_shift($rest)) {
						...
						$rec_found_in_ord_trk = "no";
						$orders_tracking_chk = db_query("Select * from orders_active_export where orders_id = '" . $_GET["id"] . "'");
					...
					<tr bgColor="#e4e4e4">
						...
						<td align="left" height="13" class="style1">
							<a href="deletelabel.php?table_name=<?php echo $wrow["tablename"];?>&id=<?php  echo $row_active["id"]; ?>">Delete</a>
						</td>
					</tr>
					<?php 
					}
			...
			<?php  if ($rec_found_in_ord_trk == "no"){?>
				<tr bgColor="#e4e4e4">
					...
					<td height="10" colspan = "3" class="style1">
						...
						<input type="button" onclick="javascript: move_warehouse_entry(<?php  echo $_GET["id"];?>); return false;"
						id="move_record" name="move_record" value="Move Warehouse" />
					</td>
					<td align="left" height="10" style="width: 100px">
						<input type="button" onclick="javascript: open_entry_frmimport(<?php  echo $_GET["id"];?>); return false;"
						id="add_record" name="add_record" value="Add Record" />
					</td>
				</tr>
			<?php  }?>		
			<!-- 
...
while ($orders_tracking = array_shift($orders_tracking_query)) 
{
		...
		if ($orders_tracking["cancel_flag"] == "yes"){
			...
			if ($the_code = '') { echo "No Data"; } else { echo $stat; unset($stat); }
		}
echo "</td>";
echo "<td class='style1'>";
// echo "For this record<br>UPS XML Says:<br>";
// echo "Activity Date: " . $monthy . "/" . $dayy . "/" . $yeary;
// echo "<br>";
// echo "Delivery Date: " . $monthx . "/" . $dayx . "/" . $yearx;
echo $monthy . "/" . $dayy . "/" . $yeary;
echo "</td>";
echo "<td class='style1'>";
if ($stat_code == 1) { echo $monthy . "/" . $dayy . "/" . $yeary; } 
if ($stat_code == 2) { echo $monthx . "/" . $dayx . "/" . $yearx; } 
if ($stat_code == 3) { echo "No Data"; } 
echo "</td>";
unset($obj);
unset($xml);
...
$chargeback = $orders_tracking["chargeback"];
		...
		switch($chargeback)
			{
				case 0:
				$cb_stat = "<a href=\"process_chargeback.php?id=".$cb_id."&orders_id=".$orders_id."\">Chargeback</a>";
				BREAK;
				case 1:
...
echo "<td class='style1' style='height: 13px' class='style1'><a href='resend_module.php?id=" . $orders_tracking['id'] . "&orders_id=" . $orders_id . "&warehouse_id=" . $orders_tracking["warehouse_id"] . "&module_name=" . $orders_tracking["module_name"] . "'>Resend</a></td>";
...
if ($orders_tracking["cancel_flag"] == "yes"){	
	...
	echo "<td>
		<a onclick='order_cancel_labels($orders_id,$orid)' style='text-decoration: underline; cursor: pointer;'>Cancel Label</a>
	</td>";
}	
echo "</tr>";
}
...
<table cellSpacing="1" cellPadding="1" width="100%" border="0">
		<tr align="middle">
			<td bgColor="#c0cdda" colspan="6">
			<font face="Arial, Helvetica, sans-serif" size="1">CUSTOMER 
			LOG HISTORY</font><font face="Arial, Helvetica, sans-serif" color="#333333" size="1">
			</font></td>
		</tr>
	<tr bgColor="#e4e4e4">
...
<td></td>
		...
		<td style="height: 13px"><td class="style1"><input type="textbox" id="other_amt" name="other_amt" size="8" onchange="update_cart()"></td>
...
<td class="style1"><input type="textbox" id="order_total" name="order_total" size="8" readonly></td>
	...
	<tr bgColor="#e4e4e4">
		...
		<input type="submit" size="8" value="Process Credit"></td>
	</tr>
