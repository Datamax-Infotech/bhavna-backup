...
$orders_tracking_query = db_query(("SELECT * FROM orders_active_export WHERE setignore != 1 and LENGTH(tracking_number) > 3 AND (fedex_status <> 'DL') ORDER BY id DESC"));
while ($orders_tracking = array_shift($orders_tracking_query)) {
	...
	$request['PackageIdentifier'] = array(
		'Value' => $orders_tracking["tracking_number"], // Replace 'XXX' with a valid tracking identifier
