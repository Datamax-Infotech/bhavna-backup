...
<body>
	...
	<div class="main_data_css">
		...
		$thispage	= "processed_credits.php"; //SET THIS TO THE NAME OF THIS FILE
				...
				?>
					...
					if ($proc == "Edit") {
						...
						if ($_REQUEST['post'] == "yes") {
							...
							$pending = FixString($_REQUEST['pending']);
							$credit_date = FixString($_REQUEST['credit_date']);
							$sql = "UPDATE ucbdb_credits SET  orders_id='$orders_id', item_name='$item_name', warehouse='$warehouse', reason_code='$reason_code', 
							quantity='$quantity',  chargeback='$chargeback', total='$total', employee='$employee', pending='$pending', credit_date='$credit_date'
						...
						?>
							...
							?>
								<DIV CLASS='PAGE_OPTIONS'>
									...
									<a href="<?php echo $thispage; ?>?id=<?php echo encrypt_url($id); ?>&delete=yes&proc=Delete&orders_id=<?php echo $orders_id; ?>&<?php echo $pagevars; ?>">Yes</a>
							...
							if ($_REQUEST['delete'] == "yes") {
								...
								if (empty($result)) {
									...
									if (!headers_sent()) {    //If headers not sent yet... then do php redirect
										header('Location: processed_credits.php?posting=yes&');
										exit;
									} else {
