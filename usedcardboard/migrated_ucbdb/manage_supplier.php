...
<head>
	<title>UCB Loop System - Supplier Management</title>
</head>
...
<body>
	...
	$sql_debug_mode = 0;
	$thispage	= "manage_supplier.php"; //SET THIS TO THE NAME OF THIS FILE
	...
	$addl_select_crit = "AND rec_type='Supplier' ORDER BY company_name"; //ADDL CRITERIA FOR SQL STATEMENTS (ADD/UPD/DEL).
	...
	if ($proc == "") {
		if ($allowaddnew == "yes") {
	?>
			<a href="<?php echo $thispage; ?>?proc=New&<?php echo $pagevars; ?>">Add New Supplier</a><br>
		<?php  }
		?>
		<?php
			...
			if ($newpage != -1) {
				...
				<A HREF="<?php echo $thispage; ?>?posting=yes&page=<?php echo $newpage; ?>&reccount=<?php echo $reccount; ?>&searchcrit=<?php echo $searchcrit; ?>&<?php echo $pagevars; ?>">
					...
					if ($myrowsel = array_shift($result)) {
						...
						echo "	<tr align='middle'><td colspan='6' class='style24' style='height: 16px'><strong>SUPPLIER SETUP</strong></td></tr>";
						echo "	<TR>";
					...
					?>
						...
						if ($proc == "New") {
							...
							echo "<a href=\"manage_supplier.php?posting=yes\">Back</a><br><br>";
							echo "<table border=0 width=780><tr><td valign=top><DIV CLASS='PAGE_STATUS'>Add New Supplier</DIV>";
							if ($_REQUEST['post'] == "yes") {
								...
								/*-- SECTION: 9994SQL --*/
								$sql = "INSERT INTO loop_warehouse ( company_name,company_address1, company_address2,company_city,company_state,company_zip,
								company_phone,company_email,company_terms,company_contact,warehouse_name,warehouse_address1,warehouse_address2,warehouse_city,warehouse_state,warehouse_zip,
								warehouse_contact,warehouse_contact_phone,warehouse_contact_email,warehouse_manager,warehouse_manager_phone,warehouse_manager_email,dock_details,
								warehouse_notes,rec_type,bs_status,other1,other2,other3 $addl_insert_crit ) VALUES ( '$company_name','$company_address1','$company_address2','$company_city',
								'$company_state','$company_zip','$company_phone','$company_email','$company_terms','$company_contact','$warehouse_name','$warehouse_address1','$warehouse_address2',
								'$warehouse_city','$warehouse_state','$warehouse_zip','$warehouse_contact','$warehouse_contact_phone','$warehouse_contact_email','$warehouse_manager',
								'$warehouse_manager_phone','$warehouse_manager_email','$dock_details','$warehouse_notes','$rec_type','$bs_status','$other1','$other2','$other3' $addl_insert_values )";
								if ($sql_debug_mode == 1) {
									echo "<BR>SQL: $sql<BR>";
								}
								$result = db_query($sql);
								if (empty($result)) {
									echo "<br><DIV CLASS='SQL_RESULTS'>Supplier Added.  <a href=\"manage_supplier.php?posting=yes\">Continue</a></DIV>";
									if (!headers_sent()) {    //If headers not sent yet... then do php redirect
										header('Location: manage_supplier.php?posting=yes');
										exit;
									} else {
										echo "<script type=\"text/javascript\">";
										echo "window.location.href=\"manage_supplier.php?posting=yes\";";
										echo "</script>";
										echo "<noscript>";
										echo "<meta http-equiv=\"refresh\" content=\"0;url=manage_supplier.php?posting=yes\" />";
										echo "</noscript>";
										exit;
									} //==== End -- Redirect	
								} else {
							...
							?>
								<FORM METHOD="POST" ACTION="<?php echo $thispage; ?>?proc=New&post=yes&<?php echo $pagevars; ?>">
									...
									<input type="hidden" name="bs_status" value="Buyer">
									<TABLE ALIGN='LEFT'>
										<TR>
					...
					?>
						...
						if ($proc == "Edit") {
							...
							if ($_REQUEST['post'] == "yes") {
								...
								$other2 = FixString($_REQUEST['other2']);
								$other3 = FixString($_REQUEST['other3']);
								$sql = "UPDATE loop_warehouse SET  company_name='$company_name', company_address1='$company_address1', company_address2='$company_address2',
 								company_city='$company_city', company_state='$company_state', company_zip='$company_zip', company_phone='$company_phone',
 								company_email='$company_email', company_terms='$company_terms', company_contact='$company_contact', warehouse_name='$warehouse_name',
 								warehouse_address1='$warehouse_address1', warehouse_address2='$warehouse_address2', warehouse_city='$warehouse_city',warehouse_state='$warehouse_state',
 								warehouse_zip='$warehouse_zip', warehouse_contact='$warehouse_contact', warehouse_contact_phone='$warehouse_contact_phone',warehouse_contact_email='$warehouse_contact_email',
 								warehouse_manager='$warehouse_manager', warehouse_manager_phone='$warehouse_manager_phone', warehouse_manager_email='$warehouse_manager_email',dock_details='$dock_details',
 								warehouse_notes='$warehouse_notes', other1='$other1', other2='$other2', other3='$other3' $addl_update_crit WHERE (id='$id') $addl_select_crit ";
								if ($sql_debug_mode == 1) {
									echo "<BR>SQL: $sql<BR>";
								}
								$result = db_query($sql);
								if (empty($result)) {
									echo "<DIV CLASS='SQL_RESULTS'>Supplier Updated.  <a href=\"manage_supplier.php?posting=yes\">Continue</a></DIV>";
					...
					?>
						...
						if ($proc == "View") {
							...
							?>
								...
								<?php
								/*-- SECTION: 9995CONFIRM --*/
								if (!$_REQUEST['delete'] || $_REQUEST['delete'] != "yes") {
								?>
									<DIV CLASS='PAGE_OPTIONS'>
										<br><br>Are you sure you want to delete this Supplier?<br><br>
