<?php 
	...
	if ($_FILES["file"]["error"] > 0)
	...
	else
		{
			...
			if(in_array(strtolower($ext), $allow_ext) )
			{
				...
				echo "Temp file: " . $_FILES["file"]["tmp_name"] . "<br />";    if (file_exists("files/" . $_FILES["file"]["name"]))
				{
	...
	if (!headers_sent()){    //If headers not sent yet... then do php redirect
			header('Location: contact_status_drill.php?id=' . $_POST['orders_id'] . '&proc=View'); exit;
	}
	else
	{
			echo "<script type=\"text/javascript\">";
			echo "window.location.href=\"contact_status_drill.php?id=" . $_POST['orders_id'] . "&proc=View\";";
			echo "</script>";
			echo "<noscript>";
			echo "<meta http-equiv=\"refresh\" content=\"0;url=contact_status_drill.php.php?id=" . $_POST['orders_id'] . "&proc=View\" />";
			echo "</noscript>"; exit;
	}//==== End -- Redirect
