...
//$orders_tracking_query = db_query(("SELECT * FROM orders_active_export WHERE setignore != 1"), db());
// $orders_tracking_query = db_query(("SELECT * FROM orders_active_export WHERE orders_id > 296789 AND orders_id < 298367 AND status != 'Delivered'"), db());
$orders_tracking_query = db_query(("SELECT * FROM orders_active_export WHERE orders_id > 297779 AND orders_id < 297799 AND status != 'Delivered'"), db());

while ($orders_tracking = array_shift($orders_tracking_query)) {
/* XML Communication with UPS */		
$y = "<?phpxml version=\"1.0\"?><AccessRequest xml:lang=\"en-US\"><AccessLicenseNumber>".$access_key."</AccessLicenseNumber><UserId>".$userid_pass."</UserId><Password>".$userid_pass."</Password></AccessRequest><?phpxml version=\"1.0\"?><TrackRequest xml:lang=\"en-US\"><Request><TransactionReference><CustomerContext>Example 1</CustomerContext><XpciVersion>1.0001</XpciVersion></TransactionReference><RequestAction>Track</RequestAction><RequestOption>".$activity."</RequestOption></Request><TrackingNumber>".$orders_tracking["tracking_number"]."</TrackingNumber></TrackRequest>";
