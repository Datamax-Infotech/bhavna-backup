<?php 
	...
	//update_shopify_tracking_no(394075, 3665193566365, 9541141, 'Fedex', $tracking_urls);
	//exit;
	...
	$sqlgetemp = "SELECT orders.orders_id, date_purchased, shopify_order_no FROM orders left JOIN orders_active_export ON orders.orders_id = orders_active_export.orders_id where shopify_order_no <> '' and (tracking_number <> '' or ubox_order_tracking_number<> '') and updated_tracking_shopify_flg = 0 and `cancel` <> 'Yes' and orders.orders_id > 391425 group by orders.orders_id";
...
// dummy 391509 - 2892711559325
	...
	while ($myrowselemp = array_shift($ressqlgetemp)) {
		...
		if ($output_data == "yes") {
			...
			echo "Order id - " . $myrowselemp["orders_id"] . " | " . $myrowselemp["shopify_order_no"] . ", tracking_number " . $track_number . " " . $carrier_code ."<br>";
			//. " " . $tracking_urls
