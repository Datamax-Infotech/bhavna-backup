...
function update_shopify_tracking_no(int $orders_id, int $shopify_order_no, string $tracking_number_org, string $carrier_code_org, string $tracking_urls): void
{
	...
	if ($shopify_order_no != "") {
		...
		if ($result) {
			...
			$shopify_fulfillment_id = "";
			$shopify_fulfillment_line_item_arr = "";
...
function getWorkdays(string $date1, string $date2, bool $workSat = false, string $patron = null): int
{
	...
	for ($i = $yearStart; $i <= $yearEnd; $i++) {
		$easter = date('Y-m-d', easter_date(intval($i)));
		list($y, $m, $g) = explode("-", $easter);
		$monday = mktime(0, 0, 0, intval(date($m)), intval(date($g)) + 1, intval(date($y)));
...
while ($myrowselemp = array_shift($ressqlgetemp)) {
	...
	$ressqlgetemp1 = db_query($sqlgetemp1);
