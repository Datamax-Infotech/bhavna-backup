	...
	*/
	...
	function tep_db_num_rows($db_query) {
	  ...
	  function db_query_old($query) {
		...
		if (!$result) {
			if ($b_debugmode) {
				$message  = '<b>Invalid query:</b><br>' . $mysqli->error . '<br><br>';
				die($message);
			}
...
function ThrowError($err_type, $err_descr, $mysqli) {
    $err_text = "";
	$dbname="";
    switch ($err_type) {
        case "9991SQLresultcount":
        case "9994SQL":
            return "<br><b>ERROR:  --> " . $mysqli->error ."</b>
...
function encryptstr($encryptValue){
...
function decryptstr($decryptValue){
