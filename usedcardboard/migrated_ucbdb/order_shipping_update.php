...
require ("mainfunctions/general-functions.php");
	...
	$customer_telephone = ""; $customer_email = ""; $order_comment = ""; $fedex_bad_add = 0; $cancel_order ="No";
	$bill_to_name = ""; $bill_to_street = ""; $bill_to_city = "";
	...
	$fedex_validate_bad_add = 0; $fedex_reply_time = ""; $fedex_search_resp_state = ""; $fedex_search_resp_classification = "";
	$fedex_search_resp_add1 = ''; $fedex_search_resp_add2 = ''; $fedex_search_resp_statecode = ''; $fedex_search_resp_city = ''; $fedex_search_resp_zip = '';
			...
			if ($product_details_tmp["products_id"] != ""){
				...
				if(in_array($ucb_prod_id, $arr_rgp))
				{
					...
					if ($rec_found == "no") {
						...
						bill_to_state = ?, bill_to_country = ?, bill_to_zip = ?";
						//echo $ttpp_query . "<br>";
						db_query($ttpp_query , array("i","s", "s", "s", "s", "s", "s", "s", "s", "s", "s", "s", "s", "s", "s", "s") , array($_REQUEST["tmp_orderid"], $new_thirdparty_mnsd,
				...
				if(!in_array($ucb_prod_id, $arr_rgp))
				{
					...
					$row_qty = array_shift(db_query($pr_qty, array("i","i"), array($_REQUEST["tmp_orderid"], $product_id_new)));
					$y=$row_qty["products_quantity"];
					for ($x=1; $x<=$y; $x++) 
					{
						$query = "SELECT kit_id FROM orders_products WHERE orders_id=? AND products_id=?";
						$row_kits = array_shift(db_query($query, array("i","i"), array($_REQUEST["tmp_orderid"],$product_id_new)));
						...
						while($row_mk_info = array_shift($res_mk_info))
						{
							...
							if ($chk_only_first_rec == "yes"){
								$res_dup_chk = db_query("Select orders_id from ".$dest_tbl_name." where orders_id = '" . $_REQUEST["tmp_orderid"] . "'", db());
