...
// UPS Specialty Department 1-800-247-9035
...
$orders_tracking_query = db_query(("SELECT * FROM orders_active_export WHERE setignore != 1 AND LENGTH(tracking_number) > 16 AND orders_id > 346000"), db());
...
//echo $stat;
if ($stat == 'Delivered') {
			$ins_sql = "update orders_active_export SET setignore = 1 where id = " . $orders_tracking["id"] ;
			db_query($ins_sql,db() );
}
if ($stat == 'In transit') {
