...
// Start the processing to read the values submitted
...
if(count($_POST))
{
	...
	foreach($process as $process_id)
	{
		...
		if(isset($arr_data[$order_id]))
			...
			$arr_data[$order_id] = array('id'=>$id, 'amount'=>$amount, 'cc_number'=>$cc_number, 'cc_expires'=>$cc_expires, 'auth_trans_id'=>$auth_trans_id, 'employee'=>$employee);	
	}
	...
	foreach($arr_data as $order_id=>$each_ord_data)
	{
		...
		// Testing - Echo out the value
		//echo $data;
		//echo "<br><br>";
		// Testing - Echo out the value
		// Clear the response just in case it is lingering from a previous transaction since DK wants a crazy loop
