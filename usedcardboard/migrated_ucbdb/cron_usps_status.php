...
db();
function curl_get_file_contents(string $URL): string|bool
{
	$c = curl_init();
...
foreach ($arrTrackNum as $arrTrackNumKey => $arrTrackNumValue) {
	...
	if ($arrTrackNumValue['tracking_number'] != '') {
		if ($arrTrackNumValue['carrier_code'] == 'fedex') {
			...
			try {
				...
				if ($response->HighestSeverity == 'SUCCESS') {
					...
					$selUboxDtsTrkngNo = db_query("SELECT ubox_order_tracking_number FROM  ubox_order_fedex_details WHERE ubox_order_tracking_number = '" . $trackingNumber . "' AND  orders_id = " . $orderID);
					...
					if (empty($selUboxDtsTrkngNo[0]['ubox_order_tracking_number'])) {
						...
						db_query("INSERT into ubox_order_fedex_details(orders_id,ubox_order_tracking_number,ubox_order_fedex_status,ubox_order_fedex_description, product_description) VALUES(" . $orderID . ",'" . $trackingNumber . "','" . $uboxOrderFedexStatus . "','" . $uboxOrderFedexDescription . "', '" . $product_details . "')");
						...
						db_query("UPDATE ubox_order_fedex_details SET ubox_order_fedex_status = '" . $uboxOrderFedexStatus . "', ubox_order_fedex_description = '" . $uboxOrderFedexDescription . "', product_description = '" . $product_details . "' WHERE ubox_order_tracking_number = '" . $trackingNumber . "' AND orders_id = " . $orderID);
					...
					db_query("Update orders set updated_tracking_ubox_shopify_flg = 1 WHERE orders_id = '" . $orderID . "'");
				} else {
		...
		if ($arrTrackNumValue['carrier_code'] == 'usps') {
			...
			if (empty($selUboxDtsTrkngNo[0]['ubox_order_tracking_number'])) {
				db_query("INSERT INTO ubox_order_fedex_details(orders_id, ubox_order_tracking_number, ubox_order_fedex_status, ubox_order_usps_description, usps_order) VALUES(" . $orderID . ", '" . $trackingNumber . "', '" . $uboxOrderUspsStatus . "', '" . $uboxOrderUspsDescription . "', 1)");
				...
				db_query("UPDATE ubox_order_fedex_details SET ubox_order_fedex_status = '" . $uboxOrderUspsStatus . "', ubox_order_usps_description = '" . $uboxOrderUspsDescription . "' WHERE ubox_order_tracking_number = " . $trackingNumber . " AND orders_id = " . $orderID);
...
foreach ($order_id_array as $order_id_array_val) {
	$update_flg = "";
	$orders_tracking_res = db_query(("SELECT ubox_order_fedex_status FROM ubox_order_fedex_details WHERE orders_id = " . $order_id_array_val . ""));
	while ($orders_tracking = array_shift($orders_tracking_res)) {
		$update_flg = "yes";
	...
	if ($update_flg == "yes") {
		db_query("UPDATE orders SET usps_tracking_delivery_status = 1 WHERE orders_id = '" . $order_id_array_val . "'");
	}
}

$datewtime = date("F j, Y, g:i a");

db_query("UPDATE tblvariable SET variablevalue = '" . $datewtime . "' WHERE variablename = 'ubox_tracking_usps_updation'");
