...
if (isset($_REQUEST["tbl"])){
	if ($_REQUEST["tbl"] == "losangeles"){
		...
		$sql_query = " orders_id = 381813 and "; 
	}
...
if ($flag == "all") {
$sql = $sql . " WHERE $sql_query (1=1)  ORDER BY id $addl_select_crit LIMIT $myrecstart, $pagenorecords";
$sqlcount = $sqlcount  . " WHERE $sql_query (1=1) $addl_select_crit LIMIT $myrecstart, $pagenorecords";
} else {
$sqlcount = $sqlcount . $sqlwhere . ") $addl_select_crit LIMIT $myrecstart, $pagenorecords";
$sql = $sql . $sqlwhere . ")  ORDER BY id$addl_select_crit LIMIT $myrecstart, $pagenorecords";
...
?>
			...
			<TD CLASS='<?php  echo $shade; ?>'>
				<a target="_blank" href="orders.php?tbl=<?php  echo $_REQUEST["tbl"];?>&id=<?php  echo $orders_id; ?>&proc=View"><?php  echo $orders_id; ?></a> 
			</TD>
			
			<?php  if ($orders_id_tmp != $orders_id){?>
				<td class='<?php  echo $shade; ?>'><a href="send_orders.php?ordersid=<?php  echo $orders_id;?>">Send to UBox</a></td>
			<?php  }else{?>
