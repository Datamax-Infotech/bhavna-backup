...
/*************************************************************************************************************
All line comment by Amarendra
Dated 12 March 2021
Submision code have place in orders.php given with file named js.txt file
/*************************************************************************************************************/
?>
<div class="container">
	...
	<?php 
		/*
		$sqlwarehouse = "SELECT * FROM orders where orders_id = " . $_GET["id"];
		...
		*/	
			...
			$products = db_query($sqlproducts,db() );
			?>
			<tr align="middle" bgColor="#e4e4e4">
				<td height="13" style="width:120px" title="<?php  echo $shipping_details;?>">Column Name</td>
			...
			<!-- 
			<form name="importfrm" method="post" action="orders_import_new_product2.php">
			-->
			<tr bgColor="#e4e4e4">
				...
				<td align="left" height="13">
					<select id="warehouseid" name="warehouseid" style="width:150px" required>
						...
						<?php  
						$w_resw = db_query("SELECT * FROM ucbdb_warehouse order by tablename",db() );		
			...
			<tr bgColor="#e4e4e4">
				...
				<td align="left">
					<select id="kitsname" name="kitsname" style="width:150px" onchange="javascript: value_set2kitid(this);" required>
						<option value=""> Select One</option>
						<?php  
						$kitsdatas = db_query("SELECT * FROM kits WHERE cancel = 'No'",db() );		
				...
				<td align="left" height="13">
					<select id="productmodule" name="productmodule" onchange="javascript: module_data_show(this);" style="width:150px" required>
						<option value=""> Select One</option>
						<?php  
						$moduledatas = db_query("SELECT * FROM `module` WHERE cancel = 'No'",db() );		
