<?php 
				...
				if ($product_details_tmp["products_id"] != ""){
					...
					if(in_array($ucb_prod_id, $arr_rgp))
					{
						...
						if ($rec_found == "no") {
							...
							//echo $ttpp_query . "<br>";
							db_query($ttpp_query , array("i","s", "s", "s", "s", "s", "s", "s", "s", "s", "s", "s", "s", "s", "s", "s") , array($_REQUEST["orders_id"], $new_thirdparty_mnsd,
					...
					if(!in_array($ucb_prod_id, $arr_rgp))
					{
						...
						$row_qty = array_shift(db_query($pr_qty, array("i","i"), array($_REQUEST["orders_id"], $product_id_new)));
						$y=$row_qty["products_quantity"];
						for ($x=1; $x<=$y; $x++) 
						{
							$query = "SELECT kit_id FROM orders_products WHERE orders_id=? AND products_id=?";
							$row_kits = array_shift(db_query($query, array("i","i"), array($_REQUEST["orders_id"],$product_id_new)));
							...
							while($row_mk_info = array_shift($res_mk_info))
							{
								...
								db();
								/*$query = "INSERT INTO ".$dest_tbl_name." SET warehouse_id=$dest_warehouse_id, orders_id=" . $_REQUEST["orders_id"] .", product_id=$product_id_new, kit_id=".$row_mk_info["kits_id"].", module_id=".$row_mk_info["module_id"];
	...
	echo "window.location.href=\"orders.php?id=" . $_REQUEST["orders_id"] .'&proc=View&searchcrit=&page=0' . "\";";
	echo "</script>";
	echo "<noscript>";
	echo "<meta http-equiv=\"refresh\" content=\"0;url=orders.php?id=" . $_REQUEST["orders_id"] .'&proc=View&searchcrit=&page=0' . "\" />";
	echo "</noscript>"; exit;
