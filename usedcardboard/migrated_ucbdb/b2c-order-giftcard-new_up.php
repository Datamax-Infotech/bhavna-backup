...
<head>
	...
	function tep_round(int $number, int $precision): float
	{
	...
	function tep_db_perform(string $table, array $data, string $action = 'insert', string $parameters = '', $link = 'db_link'): mixed
	{
		reset($data);
		if ($action == 'insert') {
	...
	function tep_db_free_result(mysqli_result $db_query): void
	{
		return mysqli_free_result($db_query);
	}

	function tep_db_fetch_fields(mysqli_result $db_query): mixed
	{
	...
	function tep_db_prepare_input(mixed $string): mixed
	{
		if (is_string($string)) {
	...
	if ($_POST["btnsubmit"] == "Submit") {
		if (isset($_POST['g-recaptcha-response']) && !empty($_POST['g-recaptcha-response'])) {
			...
			if ($responseData->success) {
				...
				$orders_products_query = db_query("SELECT * FROM products inner join products_description on products_description.products_id = products.products_id where products.products_id = " . $_POST['product_id'] . " AND products_description.language_id = 1 ", db());
				...
				$total_weight += ($product_quntity * $products_weight);
				...
				if (in_array($product_id, $arr_rgp)) {
					...
					$row = array_shift(db_query($qrystatefull, array("s"), array($cust_state)));
				...
				$email_order .= /* STORE_NAME . "\n" . */ EMAIL_SEPARATOR . "\n" .
					...
					EMAIL_TEXT_DATE_ORDERED . ' ' . date('m/d/Y H:i:s', time()) . "\n\n";
				...
				$row = array_shift(db_query($querystatefull, array("s"), array($statefull)));
				...
				if (!in_array($pr_id, $arr_rgp)) {
					...
					$row_qty = array_shift(db_query($pr_qty, array("i", "i"), array($insert_id, $pr_id)));
					...
					for ($x = 1; $x <= $y; $x++) {
						...
						$row_kits = array_shift(db_query($query, array("i", "i"), array($insert_id, $pr_id)));
			...
			<table cellSpacing="1" cellPadding="1" width="900" border="0" style="margin:0 auto">
							...
							<tr>
								...
								<td width="10%">
									<select name="product_id" id="product_id" onChange="javascript:product_price_show();">
										...
										$orders_products_query1 = db_query($sqlp1, db());
										while ($prod2 = array_shift($orders_products_query1)) {
										?>
											<option value="<?php echo $prod2['ucb_products_id'] ?>" data-price="<?php echo $prod2['shopify_products_price'] ?>">
												<?php echo $prod2['product_description'] ?>
											</option>
										<?php } ?>
								</td>
							</tr>
				...
				<tr>
					...
					<td width="50%">
						<table cellSpacing="1" cellPadding="1" border="0">
							<tr>
								<td width="100%" colspan="2" style="padding:10px 0;">Billing Address <br> Same as
									Customer Address
