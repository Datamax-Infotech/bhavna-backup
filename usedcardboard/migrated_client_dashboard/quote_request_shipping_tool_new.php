...
if ($numrows > 0) { ?>
	<?php
		//while ($gblrow = array_shift($gbl_res)) { echo $gblrow["tipstr"]; }
	?>
<?php } else {
	$qty_avail = ""; 
	$shipLat = 0;
	...
	// Added by Mooneem Jul-13-12 to Bring the green thread at top	
		...
		while ($inv = array_shift($yyyy)) {
			...
			//if (( (int)$inv["depthInch"] >= (int)$min_height) && ((int)$inv["depthInch"] <= (int)$max_height))
	  ...
	  $dt_view_qry = ""; 
		...
		while ($dt_view_row = array_shift($dt_view_res)) {
			...
			//while ($dt_view_row_inv = array_shift($dt_view_inv_res)) {
				...
				//if (( (int)$dt_view_row_inv["depthInch"] >= (int)$min_length && (int)$dt_view_row_inv["depthInch"] <= (int)$max_length)) 
			...
			//
				...
				$zipStr = "";
				if($dt_view_row_inv["location_country"] == "Canada" )
				{ 	
					$tmp_zipval = str_replace(" ", "", $dt_view_row_inv["location_zip"]);
					$zipStr= "Select * from zipcodes_canada WHERE zip = '" . $tmp_zipval . "'";
				}elseif(($dt_view_row_inv["location_country"]) == "Mexico" ){
					$zipStr= "Select * from zipcodes_mexico limit 1";
				}else {
