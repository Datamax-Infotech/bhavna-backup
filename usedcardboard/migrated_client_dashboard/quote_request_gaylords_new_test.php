...
<div class="scrollit">
	<table width="100%" border="0" cellspacing="1" cellpadding="1" class="basic_style">
		...
		?>
			<tr class="headrow2">
				<?php if ($_REQUEST['sort_g_location'] == 2 && $_REQUEST['display_view'] == 1) { ?>
				...
				<?php } ?>
				<?php //if($_REQUEST['sort_g_location'] == 3 ){ 
				if ($_REQUEST['sort_g_location'] == 2 && $_REQUEST['display_view'] == 2) {
				?>
					<?php if ($hide_record != 1) {  ?>
						<td class='display_title'>View Item</td>
					<?php  } ?>
					...
					<td class='display_title'>Favorite?</td>
				<?php } ?>
				<?php if ($_REQUEST['sort_g_location'] == 4) { ?>
					<td class='display_title'>Qty Avail NOW</td>
					<td class='display_title'>Lead Time for FTL</td>
					<td class='display_title'>Qty Available, Next 3 months</td>
					<td class='display_title'>Total Annual Volume </td>
		...
		if ($numrows > 0) {  ?>
			...
			if ($_REQUEST["gbox"] == 0) {
				...
				$qaa = "Select * from quote_request INNER JOIN quote_gaylord ON quote_request.quote_id = quote_gaylord.quote_id where quote_request.companyID = " .  $_REQUEST["ID"] . " and quote_gaylord.id = " . $_REQUEST["gbox"] . " order by quote_gaylord.id DESC";
			}
			...
			if ($qnumrows > 0) {
				while ($qgb = array_shift($qdt_view_res)) {
					...
					if ($first_load == "yes") {
						...
						// left join tmp_inventory_list_set2 on tmp_inventory_list_set2.inv_id = inventory.ID 		
						if ($_REQUEST["load_all"] == 1) {
							...
							$dk = "SELECT *, inventory.id AS I, inventory.lengthInch AS L, inventory.widthInch AS W, inventory.depthInch AS D, inventory.notes AS N, inventory.date AS DT, inventory.vendor AS V FROM inventory left join tmp_inventory_list_set2 on tmp_inventory_list_set2.trans_id = inventory.loops_id	WHERE (inventory.box_type = 'Gaylord' or inventory.box_type = 'GaylordUCB' or inventory.box_type = 'PresoldGaylord') $qty_avail_now_str and inventory.Active LIKE 'A' ORDER BY inventory.availability DESC";
						} else {
							if ($_REQUEST["display-allrec"] == 1) {
								$dk = "SELECT *, inventory.id AS I, inventory.lengthInch AS L, inventory.widthInch AS W, inventory.depthInch AS D, inventory.notes AS N, inventory.date AS DT, inventory.vendor AS V FROM inventory left join tmp_inventory_list_set2 on tmp_inventory_list_set2.trans_id = inventory.loops_id 	WHERE (inventory.box_type = 'Gaylord' or inventory.box_type = 'GaylordUCB' or inventory.box_type = 'PresoldGaylord') and (inventory.b2b_status=1.0 or inventory.b2b_status=1.1 or inventory.b2b_status=1.2) $qty_avail_now_str AND inventory.Active LIKE 'A' ORDER BY inventory.availability DESC";
							} elseif ($_REQUEST["display-allrec"] == 2) {
								$dk = "SELECT *, inventory.id AS I, inventory.lengthInch AS L, inventory.widthInch AS W, inventory.depthInch AS D, inventory.notes AS N, inventory.date AS DT, inventory.vendor AS V FROM inventory left join tmp_inventory_list_set2 on tmp_inventory_list_set2.trans_id = inventory.loops_id  WHERE (inventory.box_type = 'Gaylord' or inventory.box_type = 'GaylordUCB' or inventory.box_type = 'PresoldGaylord') and (inventory.b2b_status=1.0 or inventory.b2b_status=1.1 or inventory.b2b_status=1.2 or inventory.b2b_status=2.0 or inventory.b2b_status=2.1 or inventory.b2b_status=2.2) $qty_avail_now_str AND inventory.Active LIKE 'A' ORDER BY inventory.availability DESC";
						...
						if ($_REQUEST["load_all"] == 1) {
							//AND (b2b_status=1.0 or b2b_status=1.1 or b2b_status=1.2 or b2b_status=2.0 or b2b_status=2.1 or b2b_status=2.2)
							$dk = "SELECT *, inventory.id AS I, inventory.lengthInch AS L, inventory.widthInch AS W, inventory.depthInch AS D, inventory.notes AS N, inventory.date AS DT, inventory.vendor AS V FROM inventory WHERE (inventory.box_type = 'Gaylord' or inventory.box_type = 'GaylordUCB' or inventory.box_type = 'PresoldGaylord') and inventory.Active LIKE 'A' ORDER BY inventory.availability DESC";
						} else {
							if ($_REQUEST["display-allrec"] == 1) {
								$dk = "SELECT *, inventory.id AS I, inventory.lengthInch AS L, inventory.widthInch AS W, inventory.depthInch AS D, inventory.notes AS N, inventory.date AS DT, inventory.vendor AS V FROM inventory WHERE (inventory.box_type = 'Gaylord' or inventory.box_type = 'GaylordUCB' or inventory.box_type = 'PresoldGaylord') and (b2b_status=1.0 or b2b_status=1.1 or b2b_status=1.2) AND inventory.Active LIKE 'A' ORDER BY inventory.availability DESC";
							} elseif ($_REQUEST["display-allrec"] == 2) {
								$dk = "SELECT *, inventory.id AS I, inventory.lengthInch AS L, inventory.widthInch AS W, inventory.depthInch AS D, inventory.notes AS N, inventory.date AS DT, inventory.vendor AS V FROM inventory WHERE (inventory.box_type = 'Gaylord' or inventory.box_type = 'GaylordUCB' or inventory.box_type = 'PresoldGaylord') and (b2b_status=1.0 or b2b_status=1.1 or b2b_status=1.2 or b2b_status=2.0 or b2b_status=2.1 or b2b_status=2.2) AND inventory.Active LIKE 'A' ORDER BY inventory.availability DESC";
							} elseif ($_REQUEST["sort_g_tool2"] == 1) {
								$dk = "SELECT *, inventory.id AS I, inventory.lengthInch AS L, inventory.widthInch AS W, inventory.depthInch AS D, inventory.notes AS N, inventory.date AS DT, inventory.vendor AS V FROM inventory WHERE (inventory.box_type = 'Gaylord' or inventory.box_type = 'GaylordUCB' or inventory.box_type = 'PresoldGaylord') and (b2b_status=1.0 or b2b_status=1.1 or b2b_status=1.2) AND inventory.Active LIKE 'A' ORDER BY inventory.availability DESC";
							}
						}
					...
					while ($inv = array_shift($yyyy)) {
						...
						if (($_REQUEST["sort_g_tool2"] == 2) || ($show_rec_condition1 == "yes" && $show_rec_condition2 == "yes" && $show_rec_condition3 == "yes" && $show_rec_condition4 == "yes" && $show_rec_condition5 == "yes" && $show_rec_condition6 == "yes")) {
							...
							if ($inv["location_zip"] != "") {
								...
								$qry_loc = "select id, box_warehouse_id,vendor_b2b_rescue, next_load_available_date from loop_boxes where b2b_id=" . $inv["I"];
						...
						$to_show_rec = "n";
								...
								$hideid = db_query("SELECT id FROM clientdash_hide_items WHERE hideItems = 1 AND hide_b2bid ='" . $inv["I"] . "' AND compid = '" . $_REQUEST["ID"] . "' AND boxtype = 'g'");
								if (!empty($hideid)) {
									$to_show_rec = "n";
								}
								...
								if ($to_show_rec == "y") {
									...
									if (isset($_REQUEST["sort_g_location"]) && ($_REQUEST["sort_g_location"] == "2" || $_REQUEST["sort_g_location"] == "3" || $_REQUEST["sort_g_location"] == "4")) {
										...
										<a href='https://b2b.usedcardboardboxes.com/?id=" . urlencode(encrypt_password(get_loop_box_id($inv["I"]))) . "&compnewid=" . urlencode(encrypt_password($_REQUEST["ID"])) . "&miles=" . urlencode(encrypt_password($miles_from)) . "&param1=" . urlencode(encrypt_password($loginid)) . "' 
										target='_blank' >View Item</a></td>";
											...
											if (isset($_REQUEST["display_view"]) && $_REQUEST["display_view"] == "2") {
												if ($inv["uniform_mixed_load"] == "Mixed") {
													if ($inv["bwall_min"] == $inv["bwall_max"]) {
														$tmpTDstr =  $tmpTDstr . "<td bgColorrepl>" . $inv["bwall_min"] . " ply</td>";
													} else {
														$tmpTDstr =  $tmpTDstr . "<td bgColorrepl>" . $inv["bwall_min"] . "-" . $inv["bwall_max"] . " ply</td>";
													}
												} else {
													$tmpTDstr =  $tmpTDstr . "<td bgColorrepl>" . $inv["bwall"] . " ply</td>";
												}
											}
											...
											$fav_b2bid = $inv["I"];
											$selFavouriteDt = db_query("SELECT id FROM clientdash_favorite_items WHERE favItems = 1 AND fav_b2bid = '" . $fav_b2bid . "' AND compid = '" . $_REQUEST["ID"] . "' AND boxtype = 'g'");
											?>
											...
											if (!empty($selFavouriteDt)) {
												$tmpTDstr =  $tmpTDstr . "<td bgColorrepl><div id=fav_div_display" . $fav_b2bid . ">
											<a id='div_favourite" . $fav_b2bid . "' href='javascript:void(0);' onClick='remove_item_as_favorite(" . $fav_b2bid . ",1)'><img src='images/fav.png' width='10px' height='10px'></a></div></td>";
											} else {
												$tmpTDstr =  $tmpTDstr . "<td bgColorrepl><div id=fav_div_display" . $fav_b2bid . "><a id='div_favourite" . $inv["I"] . "' href='javascript:void(0);' onClick='add_item_as_favorite(" . $fav_b2bid . ",1)' ><img src='images/non_fav.png' width='12px' height='12px'> </a></div></td>";
											}
											//$tmpTDstr =  $tmpTDstr . "<td bgColorrepl><a id='div_favourite".$inv["I"]."' href='javascript:void(0);' onClick='add_item_as_favorite(".$fav_b2bid.")' >Add222</a></td>";
											//}
											$tmpTDstr =  $tmpTDstr . "</tr>";
										} //Display customer view ends
									}
					...
					while ($dt_view_row = array_shift($dt_view_res)) {
						...
						//if( (int)$gb["g_item_length"] == (int)$dt_view_row_inv["lengthInch"] && (int)$gb["g_item_width"] == (int)$dt_view_row_inv["widthInch"] && (int)$gb["g_item_height"] == (int)$dt_view_row_inv["depthInch"])
						...
						if (($dt_view_row["actual"] != 0) or ($dt_view_row["actual"] - $sales_order_qty != 0)) {
							...
							if (($_REQUEST["sort_g_tool2"] == 2) || ($show_rec_condition1 == "yes" && $show_rec_condition2 == "yes" && $show_rec_condition3 == "yes" && $show_rec_condition4 == "yes" && $show_rec_condition5 == "yes" && $show_rec_condition6 == "yes")) {
								...
								if ($to_show_rec1 == "y") {
									...
									$hideid = db_query("SELECT id FROM clientdash_hide_items WHERE hideItems = 1 AND hide_b2bid ='" . $dt_view_row_inv["ID"] . "' AND compid = '" . $_REQUEST["ID"] . "' AND boxtype = 'g'");
									if (!empty($hideid)) {
										$to_show_rec = "n";
									}
								...
								<?php
											//
											if (!empty($selFavouriteDt)) {
												$tmpTDstr =  $tmpTDstr . "<td bgColorrepl><div id=fav_div_display" . $fav_b2bid . "><a id='div_favourite" . $fav_b2bid . "' href='javascript:void(0);' onClick='remove_item_as_favorite(" . $fav_b2bid . ",1)' ><img src='images/fav.png' width='10px' height='10px'></a></div></td>";
											} else {
												$tmpTDstr =  $tmpTDstr . "<td bgColorrepl><div id=fav_div_display" . $fav_b2bid . "><a id='div_favourite" . $fav_b2bid . "' href='javascript:void(0);' onClick='add_item_as_favorite(" . $fav_b2bid . ",1)' ><img src='images/non_fav.png' width='12px' height='12px'> </a></div></td>";
											}
											//$tmpTDstr =  $tmpTDstr . "<td bgColorrepl><a id='div_favourite".$inv["I"]."' href='javascript:void(0);' onClick='add_item_as_favorite(".$fav_b2bid.",$btype)' >Add</a></td>";
				...
				if (isset($_REQUEST["sort_g_location"]) && ($_REQUEST["sort_g_location"] == "2" || $_REQUEST["sort_g_location"] == "3" || $_REQUEST["sort_g_location"] == "4")) {
					...
					foreach ($newLocArr as $newLocArrKey => $newLocArrValue) {
						...
						if ($_REQUEST["display_view"] == "2") {
							...
							$tmpTDstr2 =  $tmpTDstr2 . "<td>&nbsp;</td>";
							$tmpTDstr2 =  $tmpTDstr2 . "<td><a name='location_details_show' id='loc_btn_" . $vendor_b2b_rescue_id . "' onClick='show_loc_dtls(" . $vendor_b2b_rescue_id . "," . count($newLocArrValue) . "," . $_REQUEST["sort_g_location"] . " )' class='ex_col_btn' >" . get_territory($shipFrom2) . "</a></td>";
							$tmpTDstr2 =  $tmpTDstr2 . "<td>&nbsp;</td>";
							$tmpTDstr2 =  $tmpTDstr2 . "<td>&nbsp;</td>";
							$tmpTDstr2 =  $tmpTDstr2 . "<td>&nbsp;</td>";
							$tmpTDstr2 =  $tmpTDstr2 . "</tr>";
							...
							for ($i = 0; $i < count($newLocArrValue); $i++) {
								...
								$selFavouriteDt = db_query("SELECT id FROM clientdash_favorite_items WHERE favItems = 1 AND fav_b2bid = '" . $fav_b2bid . "' AND compid = '" . $_REQUEST["ID"] . "' AND boxtype = 'g'");
								if (!empty($selFavouriteDt)) {
									$tmpTDstr2 =  $tmpTDstr2 . "<td><div id=fav_div_display" . $fav_b2bid . "><a id='div_favourite" . $fav_b2bid . "' href='javascript:void(0);' onClick='remove_item_as_favorite(" . $fav_b2bid . ",1)' ><img src='images/fav.png' width='10px' height='10px'></a></div></td>";
								} else {
									$tmpTDstr2 =  $tmpTDstr2 . "<td><div id=fav_div_display" . $fav_b2bid . "><a id='div_favourite" . $fav_b2bid . "' href='javascript:void(0);' onClick='add_item_as_favorite(" . $fav_b2bid . ",1)' ><img src='images/non_fav.png' width='12px' height='12px'> </a></div></td>";
								}
								$tmpTDstr2 =  $tmpTDstr2 . "</tr>";
