...
<div class="scrollit">
	...
	<table width="100%" border="0" cellspacing="1" cellpadding="1" class="basic_style">
		...
		if ($_REQUEST["load_all"] == 1) {
			...
			$dk = "SELECT *, inventory.id AS I, inventory.lengthInch AS L, inventory.widthInch AS W, inventory.depthInch AS D, inventory.notes AS N, inventory.date AS DT, inventory.vendor AS V FROM inventory WHERE (inventory.box_type = 'PalletsUCB' or inventory.box_type = 'PalletsnonUCB') AND inventory.Active LIKE 'A' ORDER BY inventory.availability DESC";
		} else {
			if ($_REQUEST["display-allrec"] == 1) {
				$dk = "SELECT *, inventory.id AS I, inventory.lengthInch AS L, inventory.widthInch AS W, inventory.depthInch AS D, inventory.notes AS N, inventory.date AS DT, inventory.vendor AS V FROM inventory WHERE (inventory.box_type = 'PalletsUCB' or inventory.box_type = 'PalletsnonUCB') and (b2b_status=1.0 or b2b_status=1.1 or b2b_status=1.2) AND inventory.Active LIKE 'A' ORDER BY inventory.availability DESC";
			} elseif ($_REQUEST["display-allrec"] == 2) {
				$dk = "SELECT *, inventory.id AS I, inventory.lengthInch AS L, inventory.widthInch AS W, inventory.depthInch AS D, inventory.notes AS N, inventory.date AS DT, inventory.vendor AS V FROM inventory WHERE (inventory.box_type = 'PalletsUCB' or inventory.box_type = 'PalletsnonUCB') and (b2b_status=1.0 or b2b_status=1.1 or b2b_status=1.2 or b2b_status=2.0 or b2b_status=2.1 or b2b_status=2.2) AND inventory.Active LIKE 'A' ORDER BY inventory.availability DESC";
		...
		while ($inv = array_shift($yyyy)) {
			...
			//echo "Show_rec_condition - " . $inv["I"] . " " . $show_rec_condition1 . "<br>"; 
			...
			if (($dt_view_row["actual"] != 0) or ($dt_view_row["actual"] - $sales_order_qty != 0)) {
				...
				if (($_REQUEST["sort_g_tool2"] == 2) || ($show_rec_condition1 == "yes")) {
					...
					if ($bpallet_qty_tmp > 0) {
						$pallet_val = number_format((int)$dt_view_row["actual"] / (int)$bpallet_qty_tmp, 1, '.', '');
						$pallet_val_afterpo = number_format((int)$actual_po_tmp / (int)$bpallet_qty_tmp, 1, '.', '');
					}
