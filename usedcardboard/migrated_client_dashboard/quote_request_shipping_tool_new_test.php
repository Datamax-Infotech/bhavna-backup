...
<input type="hidden" name="fav_boxtype" id="fav_boxtype" value="sb">
<?php 
if (isset($_REQUEST["sort_g_location_shipping"]) && ($_REQUEST["sort_g_location_shipping"]=="2" || $_REQUEST["sort_g_location_shipping"]=="3" || $_REQUEST["sort_g_location_shipping"]=="4" )) { 
?>
	<tr class="headrow2">
		<?php if($_REQUEST['sort_g_location_shipping'] == 2 && $_REQUEST['display_view'] == 1){ ?>
		...
		<?php } ?>
		<?php //if($_REQUEST['sort_g_location_shipping'] == 3 ){ ?>
		<?php if($_REQUEST['sort_g_location_shipping'] == 2 && $_REQUEST['display_view'] == 2){ ?>
			<?php   if($hide_record != 1){  ?>
			<td class='display_title'>View Item</td>
			<?php } ?>
			<td class='display_title'>Qty Avail NOW</td>		
			...
			<td class='display_title'>Favorite?</td>
		<?php } ?>
		<?php if($_REQUEST['sort_g_location_shipping'] == 4 ){ ?>			
			<td class='display_title'>Qty Avail NOW</td>			
			<td class='display_title'>Lead Time for FTL</td>				
			<td class='display_title'>Qty Available, Next 3 months</td>		
			<td class='display_title'>Total Annual Volume	</td>					
...
<?php } else {
	...
	if ($_REQUEST["gbox"] == 0){
		...
		$qaa = "Select * from quote_request INNER JOIN quote_shipping_boxes ON quote_request.quote_id = quote_shipping_boxes.quote_id where quote_request.companyID = " .  $_REQUEST["ID"] . " and quote_shipping_boxes.id = " . $_REQUEST["gbox"] . " order by quote_shipping_boxes.id DESC";
	}		
	//echo $qaa;
    $qdt_view_res = db_query($qaa );
    $qnumrows=tep_db_num_rows($qdt_view_res);
	$arrCombineItemView_shipping = array();
	$MGArray = array();
	$MGArray1 = array();
	$MGArray2 = array();
	$inv_id_list = "";
    ...
    if($qnumrows>0){
		while ($qgb = array_shift($qdt_view_res)) {
		    ...
		    $aa = "Select * from quote_shipping_boxes Where quote_id = " . $qgb["quote_id"];
	        $MGArray = array();
			$x=0; $bg = "#f4f4f4";
	        $dt_view_res = db_query($aa) ;
	        $sb = array_shift($dt_view_res);
			$sb_length = $sb["sb_item_length"];
			$sb_width = $sb["sb_item_width"];
			$sb_height = $sb["sb_item_height"];
			...
			if ($first_load == "yes"){
				...
				if ($_REQUEST["load_all"] == 1){
					//(b2b_status=1.0 or b2b_status=1.1 or b2b_status=1.2 or b2b_status=2.0 or b2b_status=2.1 or b2b_status=2.2)
					$dk = "SELECT *, inventory.id AS I, inventory.lengthInch AS L, inventory.widthInch AS W, inventory.depthInch AS D, inventory.notes AS N, inventory.date AS DT, inventory.vendor AS V FROM inventory left join tmp_inventory_list_set2 on tmp_inventory_list_set2.trans_id = inventory.loops_id	WHERE (inventory.box_type = 'Medium' or inventory.box_type = 'Large' or inventory.box_type = 'Xlarge' or inventory.box_type = 'Boxnonucb' or inventory.box_type = 'Box' or inventory.box_type = 'Presold') $qty_avail_now_str AND inventory.Active LIKE 'A' ORDER BY inventory.availability DESC";
				}else{
					if ($_REQUEST["display-allrec"] == 1) {
						$dk = "SELECT *, inventory.id AS I, inventory.lengthInch AS L, inventory.widthInch AS W, inventory.depthInch AS D, inventory.notes AS N, inventory.date AS DT, inventory.vendor AS V FROM inventory  left join tmp_inventory_list_set2 on tmp_inventory_list_set2.trans_id = inventory.loops_id	WHERE (inventory.box_type = 'Medium' or inventory.box_type = 'Large' or inventory.box_type = 'Xlarge' or inventory.box_type = 'Boxnonucb' or inventory.box_type = 'Box' or inventory.box_type = 'Presold') and (inventory.b2b_status=1.0 or inventory.b2b_status=1.1 or inventory.b2b_status=1.2) $qty_avail_now_str AND inventory.Active LIKE 'A' ORDER BY inventory.availability DESC";
					}
					elseif($_REQUEST["display-allrec"] == 2){
						$dk = "SELECT *, inventory.id AS I, inventory.lengthInch AS L, inventory.widthInch AS W, inventory.depthInch AS D, inventory.notes AS N, inventory.date AS DT, inventory.vendor AS V FROM inventory left join tmp_inventory_list_set2 on tmp_inventory_list_set2.trans_id = inventory.loops_id	 WHERE (inventory.box_type = 'Medium' or inventory.box_type = 'Large' or inventory.box_type = 'Xlarge' or inventory.box_type = 'Boxnonucb' or inventory.box_type = 'Box' or inventory.box_type = 'Presold') and (inventory.b2b_status=1.0 or inventory.b2b_status=1.1 or inventory.b2b_status=1.2 or inventory.b2b_status=2.0 or inventory.b2b_status=2.1 or inventory.b2b_status=2.2) $qty_avail_now_str AND inventory.Active LIKE 'A' ORDER BY inventory.availability DESC";
					}
				...
				if ($_REQUEST["load_all"] == 1){
					//(b2b_status=1.0 or b2b_status=1.1 or b2b_status=1.2 or b2b_status=2.0 or b2b_status=2.1 or b2b_status=2.2)
					$dk = "SELECT *, inventory.id AS I, inventory.lengthInch AS L, inventory.widthInch AS W, inventory.depthInch AS D, inventory.notes AS N, inventory.date AS DT, inventory.vendor AS V FROM inventory WHERE (inventory.box_type = 'Medium' or inventory.box_type = 'Large' or inventory.box_type = 'Xlarge' or inventory.box_type = 'Boxnonucb' or inventory.box_type = 'Box' or inventory.box_type = 'Presold') AND inventory.Active LIKE 'A' ORDER BY inventory.availability DESC";
				}else{
					if ($_REQUEST["display-allrec"] == 1) {
						$dk = "SELECT *, inventory.id AS I, inventory.lengthInch AS L, inventory.widthInch AS W, inventory.depthInch AS D, inventory.notes AS N, inventory.date AS DT, inventory.vendor AS V FROM inventory WHERE (inventory.box_type = 'Medium' or inventory.box_type = 'Large' or inventory.box_type = 'Xlarge' or inventory.box_type = 'Boxnonucb' or inventory.box_type = 'Box' or inventory.box_type = 'Presold') and (b2b_status=1.0 or b2b_status=1.1 or b2b_status=1.2) AND inventory.Active LIKE 'A' ORDER BY inventory.availability DESC";
					}
					elseif($_REQUEST["display-allrec"] == 2){
						$dk = "SELECT *, inventory.id AS I, inventory.lengthInch AS L, inventory.widthInch AS W, inventory.depthInch AS D, inventory.notes AS N, inventory.date AS DT, inventory.vendor AS V FROM inventory WHERE (inventory.box_type = 'Medium' or inventory.box_type = 'Large' or inventory.box_type = 'Xlarge' or inventory.box_type = 'Boxnonucb' or inventory.box_type = 'Box' or inventory.box_type = 'Presold') and (b2b_status=1.0 or b2b_status=1.1 or b2b_status=1.2 or b2b_status=2.0 or b2b_status=2.1 or b2b_status=2.2) AND inventory.Active LIKE 'A' ORDER BY inventory.availability DESC";
					}
					elseif($_REQUEST["sort_g_tool2"] == 1){
						$dk = "SELECT *, inventory.id AS I, inventory.lengthInch AS L, inventory.widthInch AS W, inventory.depthInch AS D, inventory.notes AS N, inventory.date AS DT, inventory.vendor AS V FROM inventory WHERE (inventory.box_type = 'Medium' or inventory.box_type = 'Large' or inventory.box_type = 'Xlarge' or inventory.box_type = 'Boxnonucb' or inventory.box_type = 'Box' or inventory.box_type = 'Presold') and (b2b_status=1.0 or b2b_status=1.1 or b2b_status=1.2) AND inventory.Active LIKE 'A' ORDER BY inventory.availability DESC";
					}
			...
			while ($inv = array_shift($yyyy)) {
				...
				if (( (int)$inv["lengthInch"] >= (int)$min_length && (int)$inv["lengthInch"] <= (int)$max_length) && ( (int)$inv["widthInch"] >= (int)$min_width && (int)$inv["widthInch"] <= (int)$max_width) && ( (int)$inv["depthInch"] >= (int)$min_height && (int)$inv["depthInch"] <= (int)$max_height)) 
	            {
					$show_rec_condition1 = "yes";
	            }
				if (( (int)$inv["bheight_min"] >= (int)$min_height) && ((int)$inv["bheight_max"] <= (int)$max_height))
	            {
					$show_rec_condition1 = "yes";
	            }
			
				$show_rec_condition2 = "no";
				//echo "cubicFeet - " . (double)$inv["cubicFeet"] . " = " . (double)$sb_cubic_footage_min . " | " . (double)$sb_cubic_footage_max . "<br>";
				...
				if ($inv["uniform_mixed_load"] == "Mixed"){
					if($sb_wall_1=="Yes" && ($inv["bwall_min"] >= 1 && $inv["bwall_max"] <= 1)){
						$count=$count+1;
						$show_rec_condition3 = "yes";
					}
					if ( $sb_wall_2 == "Yes" && ($inv["bwall_min"] >= 2 && $inv["bwall_max"] <= 2)){
						$count=$count+1;
						$show_rec_condition3 = "yes";
					}
				}else{
				...
				if($sb_no_top=="Yes" && ((int) $inv["top_nolid"] == 1))		
				{
					$count=$count+1;
				...
				if($sb_full_flap_top=="Yes" && ((int) $inv["top_full"] == 1))		
				{
					$count=$count+1;
					$show_rec_condition4 = "yes";
				}
				if($sb_partial_flap_top=="Yes" && ((int) $inv["top_partial"] == 1))		
				{
					$count=$count+1;
					$show_rec_condition4 = "yes";
				}
				...
				if ($sb_vents_okay=="" && (int) $inv["vents_yes"] == 0) {
						$count=$count+1;
				...
				if ($sb_vents_okay=="Yes") {
					$count=$count+1;
					$show_rec_condition6 = "yes";
				}else{
					$tipcount_match_str .= "Vents: Yes (V) missing<br>";
				}
				//echo "Show_rec_condition - " . $inv["I"] . " " . $_REQUEST["sort_g_tool2"] . " " . $show_rec_condition1 . " " . $show_rec_condition2 . " " . $show_rec_condition3 . " " . $show_rec_condition4 . " " . $show_rec_condition5 . " " . $show_rec_condition6 . "<br>"; 
				//($_REQUEST["sort_g_tool2"] == 1) || 
				if (($_REQUEST["sort_g_tool2"] == 2) || ($show_rec_condition1 == "yes" && $show_rec_condition2 == "yes" && $show_rec_condition3 == "yes" && $show_rec_condition4 == "yes" && $show_rec_condition5 == "yes" && $show_rec_condition6 == "yes")) {		
					// Added by Mooneem Jul-13-12 to Bring the green thread at top	
					...
					$bpallet_qty= 0; $boxes_per_trailer= 0;	$box_type = "";	$loop_id = 0;		
					$ship_cdata_ltl = ''; $pickup_cdata_allowed = '';
					...
					if ($inv["location_zip"] != "")		
					{
						...
						while ($loc_res = array_shift($dt_view)) 
						{
							...
							if (isset($_REQUEST["sort_g_location_shipping"]) && ($_REQUEST["sort_g_location_shipping"]=="2" || $_REQUEST["sort_g_location_shipping"]=="3" )) {
						...
						if ($to_show_rec == "y")
						{
							...
							$estimated_next_load = ""; $b2bstatuscolor = ""; $expected_loads_per_mo = "";
							...
							if (isset($_REQUEST["sort_g_location_shipping"]) && ($_REQUEST["sort_g_location_shipping"]=="2" || $_REQUEST["sort_g_location_shipping"]=="3" || $_REQUEST["sort_g_location_shipping"]=="4" )) {
								...
								$arrCombineItemView_shipping[$inv["ID"]]['ID'] = $inv["ID"];
								$arrCombineItemView_shipping[$inv["ID"]]['I'] = $inv["I"];
								$arrCombineItemView_shipping[$inv["ID"]]['qty'] = $qty;								
								$arrCombineItemView_shipping[$inv["ID"]]['ship_from'] = $ship_from;
								$arrCombineItemView_shipping[$inv["ID"]]['vendor_b2b_rescue_id'] =$vendor_b2b_rescue_id;
								$arrCombineItemView_shipping[$inv["ID"]]['ReqId'] = $_REQUEST["ID"];
								$arrCombineItemView_shipping[$inv["ID"]]['expected_loads_per_mo'] = $expected_loads_per_mo;
								$arrCombineItemView_shipping[$inv["ID"]]['box_warehouse_id'] = $box_warehouse_id;
								$arrCombineItemView_shipping[$inv["ID"]]['next_load_available_date'] = $next_load_available_date;
								$arrCombineItemView_shipping[$inv["ID"]]['estimated_next_load'] = $estimated_next_load;
								$arrCombineItemView_shipping[$inv["ID"]]['boxes_per_trailer'] = number_format($boxes_per_trailer,0);
								$arrCombineItemView_shipping[$inv["ID"]]['b2b_online_fob'] = $b2b_online_fob;
								$arrCombineItemView_shipping[$inv["ID"]]['miles_from'] = $miles_from;
								$arrCombineItemView_shipping[$inv["ID"]]['b2bstatuscolor'] = $b2bstatuscolor;
								$arrCombineItemView_shipping[$inv["ID"]]['b2bstatus_name'] = $b2bstatus_name;
								$arrCombineItemView_shipping[$inv["ID"]]['length'] = $length;
								$arrCombineItemView_shipping[$inv["ID"]]['width'] = $width;
								$arrCombineItemView_shipping[$inv["ID"]]['depth'] = $depth;
								$arrCombineItemView_shipping[$inv["ID"]]['wall'] = $wall;
								$arrCombineItemView_shipping[$inv["ID"]]['description'] = $inv["description"];
								$arrCombineItemView_shipping[$inv["ID"]]['vendor_name'] = $vendor_name;
								$arrCombineItemView_shipping[$inv["ID"]]['ownername'] = $ownername;
								$arrCombineItemView_shipping[$inv["ID"]]['bwall'] = $inv["bwall"];
								$arrCombineItemView_shipping[$inv["ID"]]['ship_from2'] = $ship_from2;
								$arrCombineItemView_shipping[$inv["ID"]]['after_po_val'] = $after_po_val;
								$arrCombineItemView_shipping[$inv["ID"]]['boxes_per_trailer'] = $boxes_per_trailer;
								$arrCombineItemView_shipping[$inv["ID"]]['miles_away_color'] = $miles_away_color;
								$arrCombineItemView_shipping[$inv["ID"]]['bpallet_qty'] = $bpallet_qty;
								$arrCombineItemView_shipping[$inv["ID"]]['b2b_cost'] = $b2b_cost;
								$arrCombineItemView_shipping[$inv["ID"]]['ship_ltl'] = $ship_ltl;
								$arrCombineItemView_shipping[$inv["ID"]]['prepay'] = $prepay;
								$arrCombineItemView_shipping[$inv["ID"]]['contracted'] = $contracted;
								$arrCombineItemView_shipping[$inv["ID"]]['b_urgent'] = $b_urgent;
								$arrCombineItemView_shipping[$inv["ID"]]['N'] = $inv["N"];
								$arrCombineItemView_shipping[$inv["ID"]]['DT'] = $inv["DT"];
								$arrCombineItemView_shipping[$inv["ID"]]['distC'] = $distC;
								$arrCombineItemView_shipping[$inv["ID"]]['box_urgent'] = $inv["box_urgent"];
								$arrCombineItemView_shipping[$inv["ID"]]['lead_time'] = $inv["lead_time"];
								$arrCombineItemView_shipping[$inv["ID"]]['pickup_cdata_allowed'] = $pickup_cdata_allowed;
								$arrCombineItemView_shipping[$inv["ID"]]['ship_cdata_ltl'] = $ship_cdata_ltl;
								$arrCombineItemView_shipping[$inv["ID"]]['qty_avail_3month'] = $qty_avail_3month;
								$arrCombineItemView_shipping[$inv["ID"]]['annual_volume'] = $annual_volume;
								...
								if (isset($_REQUEST["display_view"]) && $_REQUEST["display_view"]=="1") {
									...
									$font_blue = ""; $font_blue1 = "";
									if ($after_po_val < 0) {
										$font_blue = "<font color='blue'>";
										$font_blue1 = "</font>";
										$tmpTDstr =  $tmpTDstr . "<font color='blue'>" . number_format((float)$after_po_val,0) . $pallet_val_afterpo . " (" . $expected_loads_per_mo . ")</font></td>";
									}else if ($after_po_val == 0 && $expected_loads_per_mo == 0) {
										$tmpTDstr =  $tmpTDstr . "<font color='black'>" . number_format((float)$after_po_val,0) . "</font></td>";
									}else if ($after_po_val >= $boxes_per_trailer) {
										$tmpTDstr =  $tmpTDstr . "<font color='green'>" . number_format((float)$after_po_val,0) . $pallet_val_afterpo . " (" . $expected_loads_per_mo . ")</font></td>";
									} else { 
										$tmpTDstr =  $tmpTDstr . "<font color='black'>" . number_format((float)$after_po_val,0) . $pallet_val_afterpo . " (" . $expected_loads_per_mo . ")</font></td>";
									}
									...
									if ((float)str_replace(",", "", $boxes_per_trailer) <> 0 ){
								...
								if (isset($_REQUEST["display_view"]) && $_REQUEST["display_view"]=="2") {
									$tmpTDstr = "<tr  >"; 
									if($hide_record != 1){
										$tmpTDstr =  $tmpTDstr . "<td bgColorrepl width='70px'>
										<a href='https://b2b.usedcardboardboxes.com/?id=". urlencode(encrypt_password(get_loop_box_id($inv["I"]))) 
										."&compnewid=". urlencode(encrypt_password($_REQUEST["ID"])) ."&miles=" . urlencode(encrypt_password($miles_from)) . "&param1=" . urlencode(encrypt_password($loginid)) . "' target='_blank' >View Item</a></td>";
									}
									$tmpTDstr =  $tmpTDstr . "<td bgColorrepl>";
									$font_blue = ""; $font_blue1 = "";
									if ($after_po_val < 0) {
									...
									//if($_REQUEST["client_flg"]==1){
								  	...
								  	$fav_b2bid=$inv["I"];
								  	$selFavouriteDt = db_query("SELECT id FROM clientdash_favorite_items WHERE favItems = 1 AND fav_b2bid = '".$fav_b2bid."' AND compid = '".$_REQUEST["ID"]."' AND boxtype = 'sb'");
								  	?>
									...
									if(!empty($selFavouriteDt)){
										$tmpTDstr =  $tmpTDstr . "<td bgColorrepl><div id=fav_div_display_ship".$fav_b2bid."><a id='div_favourite".$inv["I"]."' href='javascript:void(0);' onClick='remove_item_as_favorite(".$fav_b2bid.",2)'><img src='images/fav.png' width='10px' height='10px'></a></div></td>";
									}else{
										$tmpTDstr =  $tmpTDstr . "<td bgColorrepl><div id=fav_div_display_ship".$fav_b2bid."><a id='div_favourite".$inv["I"]."' href='javascript:void(0);' onClick='add_item_as_favorite(".$fav_b2bid.",2)' ><img src='images/non_fav.png' width='12px' height='12px'> </a></div></td>";
									}
									//$tmpTDstr =  $tmpTDstr . "<td bgColorrepl><a id='div_favourite".$inv["I"]."' href='javascript:void(0);' onClick='add_item_as_favorite(".$inv["I"].", $btype)' >Add</a></td>";
									$tmpTDstr =  $tmpTDstr . "</tr>";
							  	//}
			...
			if ($_REQUEST["load_all"] == 1){
				//(b2b_status=1.0 or b2b_status=1.1 or b2b_status=1.2 or b2b_status=2.0 or b2b_status=2.1 or b2b_status=2.2)
				$dt_view_qry = "SELECT * from tmp_inventory_list_set2 where wid <> 238 and (type_ofbox = 'Medium' or type_ofbox = 'Large' or type_ofbox = 'Xlarge' or type_ofbox = 'Boxnonucb' or type_ofbox = 'Box' or type_ofbox = 'Presold') order by warehouse, type_ofbox, Description";
			}else{
				if ($_REQUEST["display-allrec"] == 1) {
					$dt_view_qry = "SELECT * from tmp_inventory_list_set2 where wid <> 238 and actual > 0 and (type_ofbox = 'Medium' or type_ofbox = 'Large' or type_ofbox = 'Xlarge' or type_ofbox = 'Boxnonucb' or type_ofbox = 'Box' or type_ofbox = 'Presold') and (b2b_status=1.0 or b2b_status=1.1 or b2b_status=1.2) order by warehouse, type_ofbox, Description";
				}
				if ($_REQUEST["display-allrec"] == 2 ) {
					$dt_view_qry = "SELECT * from tmp_inventory_list_set2 where wid <> 238 and actual > 0 and (type_ofbox = 'Medium' or type_ofbox = 'Large' or type_ofbox = 'Xlarge' or type_ofbox = 'Boxnonucb' or type_ofbox = 'Box' or type_ofbox = 'Presold') and (b2b_status=1.0 or b2b_status=1.1 or b2b_status=1.2 or b2b_status=2.0 or b2b_status=2.1 or b2b_status=2.2) order by warehouse, type_ofbox, Description";
				}
				 if ($_REQUEST["sort_g_tool2"] == 1 ) {
					$dt_view_qry = "SELECT * from tmp_inventory_list_set2 where wid <> 238 and (type_ofbox = 'Medium' or type_ofbox = 'Large' or type_ofbox = 'Xlarge' or type_ofbox = 'Boxnonucb' or type_ofbox = 'Box' or type_ofbox = 'Presold') and (b2b_status=1.0 or b2b_status=1.1 or b2b_status=1.2) order by warehouse, type_ofbox, Description";
				}
			}
			db_b2b();
			...
			while ($dt_view_row = array_shift($dt_view_res)) {
				...
				$b2bid_tmp = 0; $boxes_per_trailer_tmp = 0; $bpallet_qty_tmp = 0; $vendor_id = 0; $vendor_b2b_rescue_id = 0;
				...
				$show_rec_condition1 = "no"; $show_rec_condition2 = "no"; $show_rec_condition3 = "no"; $show_rec_condition4 = "no";
				$show_rec_condition5 = "no"; $show_rec_condition6 = "no"; $show_rec_condition7 = "no"; $show_rec_condition8 = "no";
				...
				//while ($dt_view_row_inv = array_shift($dt_view_inv_res)) {
					...
					if (( (int)$dt_view_row_inv["lengthInch"] >= (int)$min_length && (int)$dt_view_row_inv["lengthInch"] <= (int)$max_length) && ( (int)$dt_view_row_inv["widthInch"] >= (int)$min_width && (int)$dt_view_row_inv["widthInch"] <= (int)$max_width) && ( (int)$dt_view_row_inv["depthInch"] >= (int)$min_height && (int)$dt_view_row_inv["depthInch"] <= (int)$max_height)) 
					{
						$show_rec_condition1 = "yes";
					}
					if (( (int)$dt_view_row_inv["blength_min"] >= (int)$min_length && (int)$dt_view_row_inv["blength_max"] <= (int)$max_length)	&& ( (int)$dt_view_row_inv["bwidth_min"] >= (int)$min_width && (int)$dt_view_row_inv["bwidth_max"] <= (int)$max_width) && ( (int)$dt_view_row_inv["bheight_min"] >= (int)$min_height && (int)$dt_view_row_inv["bheight_max"] <= (int)$max_height)) 
					{
						$show_rec_condition1 = "yes";
					}
					
					$show_rec_condition2 = "no";
					if (( (int)$dt_view_row_inv["cubicFeet"] >= (int)$sb_cubic_footage_min) && ((int)$dt_view_row_inv["cubicFeet"] <= (int)$sb_cubic_footage_max))
					{
						$show_rec_condition2 = "yes";
					}
					...
					$count = 0;
					$tipcount_match_str = "";
					if($sb_wall_1=="Yes" && $dt_view_row_inv["bwall"] == 1){
						$count=$count+1;
						$show_rec_condition3 = "yes";
					}
					if($sb_wall_2=="Yes" && $dt_view_row_inv["wall_2"] == 1){
						$count=$count+1;
						$show_rec_condition3 = "yes";
					}
					//
					$show_rec_condition4 = "no";
					if($sb_no_top=="Yes" && ((int) $dt_view_row_inv["top_nolid"] == 1))		
					{
						$count=$count+1;
						$show_rec_condition4 = "yes";
					}
					...
					if($sb_full_flap_top=="Yes" && ((int) $dt_view_row_inv["top_full"] == 1))		
					{
						$count=$count+1;
						$show_rec_condition4 = "yes";
					}
					if($sb_partial_flap_top=="Yes" && ((int) $dt_view_row_inv["top_partial"] == 1))		
					{
						$count=$count+1;
						$show_rec_condition4 = "yes";
					}
					//
					$show_rec_condition5 = "no";
					if($sb_no_bottom=="Yes" && ((int) $dt_view_row_inv["bottom_no"] == 1))		
					{
						$count=$count+1;
						$show_rec_condition5 = "yes";
					}
					...
					if($sb_full_flap_bottom=="Yes" && ((int) $dt_view_row_inv["bottom_fullflap"] == 1))		
					{
						$count=$count+1;
						$show_rec_condition5 = "yes";
					}
					...
					if($sb_partial_flap_bottom=="Yes" && ((int) $dt_view_row_inv["bottom_partial"] == 1))
					{
						$count=$count+1;
						$show_rec_condition5 = "yes";
					}
				//
					...
					$show_rec_condition6 = "no";	
					if ($sb_vents_okay=="" && (int) $dt_view_row_inv["vents_yes"] == 0) {
						$count=$count+1;
						$show_rec_condition6 = "yes";	
					}else{
						$tipcount_match_str .= "Vents: No (N) missing<br>";
						...
						$tmpTDstr =  $tmpTDstr . "<td bgColorrepl>" . $pickup_cdata_allowed . "</td>";
							...
							//if($_REQUEST["client_flg"]==1){
								...
								<?php
								  //
								if(!empty($selFavouriteDt)){
									$tmpTDstr =  $tmpTDstr . "<td bgColorrepl><div id=fav_div_display_ship".$fav_b2bid."><a id='div_favourite".$fav_b2bid."' href='javascript:void(0);' onClick='remove_item_as_favorite(".$fav_b2bid.",2)'><img src='images/fav.png' width='10px' height='10px'></a></div></td>";
								}else{
									$tmpTDstr =  $tmpTDstr . "<td bgColorrepl><div id=fav_div_display_ship".$fav_b2bid."><a id='div_favourite".$fav_b2bid."' href='javascript:void(0);' onClick='add_item_as_favorite(".$fav_b2bid.",2)' ><img src='images/non_fav.png' width='12px' height='12px'> </a></div></td>";
								}
		...
		if (isset($_REQUEST["sort_g_location_shipping"]) && ($_REQUEST["sort_g_location_shipping"]=="2" || $_REQUEST["sort_g_location_shipping"]=="3" || $_REQUEST["sort_g_location_shipping"]=="4" )) {
			if($_REQUEST["sort_g_location_shipping"]=="4"){
				foreach($arrCombineItemView_shipping as $arrCombineItemViewKey => $arrCombineItemViewVal) {	
					$tmpTDstr1 = "<tr bgColorrepl >";
					$tmpTDstr1 =  $tmpTDstr1 . "<td>&nbsp;</td>";
			...
			$newLocArr = array();
			foreach($arrCombineItemView_shipping as $k => $v) {
			...
			foreach ($newLocArr as $newLocArrKey => $newLocArrValue) {
				...
				if($_REQUEST["sort_g_location_shipping"]=="2"){
					...
					$tmpTDstr2 =  $tmpTDstr2 . "<td>&nbsp;</td>";
					$tmpTDstr2 =  $tmpTDstr2 . "<td>&nbsp;</td>";
					$tmpTDstr2 =  $tmpTDstr2 . "<td><a name='location_details_show' id='loc_btn_".$vendor_b2b_rescue_id."' onClick='show_loc_dtls(".$vendor_b2b_rescue_id.",".count($newLocArrValue).",".$_REQUEST["sort_g_location_shipping"]." )' class='ex_col_btn' >" . $shipFrom ."</a></td>";
				...
				//if($_REQUEST["sort_g_location_shipping"]=="3"){
				if($_REQUEST["display_view"]=="2"){
					...
					$tmpTDstr2 =  $tmpTDstr2 . "<td>&nbsp;</td>";
					$tmpTDstr2 =  $tmpTDstr2 . "<td><a name='location_details_show' id='loc_btn_".$vendor_b2b_rescue_id."' onClick='show_loc_dtls(".$vendor_b2b_rescue_id.",".count($newLocArrValue).",".$_REQUEST["sort_g_location_shipping"]." )' class='ex_col_btn' >" . get_territory($shipFrom2) ."</a></td>";
					$tmpTDstr2 =  $tmpTDstr2 . "<td>".$ship_cdata_ltl."</td>";
					$tmpTDstr2 =  $tmpTDstr2 . "<td>".$pickup_cdata_allowed."</td>";
					$tmpTDstr2 =  $tmpTDstr2 . "<td>&nbsp;</td>";
					$tmpTDstr2 =  $tmpTDstr2 . "</tr>";
					...
					for($i = 0; $i < count($newLocArrValue); $i++ ){
						...
						$tmpTDstr2 =  $tmpTDstr2 . "<tr class='".$classChild."' id='loc_sub_table_".$i."_".$vendor_b2b_rescue_id."' style='display: none;'>";
						if($hide_record != 1){
							$tmpTDstr2 =  $tmpTDstr2 . "<td width='70px'><a href='https://b2b.usedcardboardboxes.com/?id=". urlencode(encrypt_password(get_loop_box_id($newLocArrValue[$i]['I']))) ."&compnewid=". urlencode(encrypt_password($_REQUEST["ID"])) ."&miles=" . urlencode(encrypt_password($newLocArrValue[$i]['miles_from'])) . "&param1=" . urlencode(encrypt_password($loginid)) . "' target='_blank' >View Item</a></td>"; 
						}
						$font_blue = ""; $font_blue1 = "";	
						if ($newLocArrValue[$i]['after_po_val'] < 0) {
							$font_blue = "<font color='blue'>";
						...
						$selFavouriteDt = db_query("SELECT id FROM clientdash_favorite_items WHERE favItems = 1 AND fav_b2bid = '".$fav_b2bid."' AND compid = '".$_REQUEST['compnewid']."' AND boxtype = 'sb'");
						if(!empty($selFavouriteDt)){
							$tmpTDstr2 =  $tmpTDstr2 . "<td><div id=fav_div_display_ship".$fav_b2bid."><a id='div_favourite".$fav_b2bid."' href='javascript:void(0);' onClick='remove_item_as_favorite(".$fav_b2bid.",2)'><img src='images/fav.png' width='10px' height='10px'></a></div></td>";
						}else{
							$tmpTDstr2 =  $tmpTDstr2 . "<td><div id=fav_div_display_ship".$fav_b2bid."><a id='div_favourite".$fav_b2bid."' href='javascript:void(0);' onClick='add_item_as_favorite(".$fav_b2bid.",2)' ><img src='images/non_fav.png' width='12px' height='12px'> </a></div></td>";
						}						
						$tmpTDstr2 =  $tmpTDstr2 . "</tr>";						
						$childRow++;
					}
