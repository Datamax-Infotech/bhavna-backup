				...
				while ($qgb = array_shift($qdt_view_res)) {
							...
							if ($inv["location_zip"] != "") {
								...
								if ($to_show_rec == "y") {
									...
									if (isset($_REQUEST["display_view"]) && $_REQUEST["display_view"] == "1") {
										$tmpTDstr = "<tr  >";
										$tmpTDstr =  $tmpTDstr . "<td bgColorrepl><a href='https://b2bquote.usedcardboardboxes.com/?id=" . get_loop_box_id($inv["I"]) . "' target='_blank' >View Item</a></td>";
					...
					while ($dt_view_row = array_shift($dt_view_res)) {
						...
						if ($dt_view_row_inv["location_zip"] != "") {
							...
							$distC = 2 * atan2(sqrt($distA), sqrt(1 - $distA));
							//echo $distC . "g <br/>";
							//$tipStr = $tipStr . " Distance: ". $miles_from . " miles<BR>";

							//$tipStr = $tipStr . getinventory_change(get_loop_box_id($dt_view_row_inv["I"]));

							// Added by Mooneem Jul-13-12 to Bring the green thread at top	
							//$tipStr = preg_replace( "/'/", "\'", $tipStr );
							...
							if (($_REQUEST["sort_g_tool2"] == 2) || ($show_rec_condition1 == "yes")) {
								...
								if ($to_show_rec1 == "y") {
									...
									if ($to_show_rec == "y") {
										...
										if (isset($_REQUEST["display_view"]) && $_REQUEST["display_view"] == "1") {
											$tmpTDstr = "<tr  >";
											$tmpTDstr =  $tmpTDstr . "<td bgColorrepl><a href='https://b2bquote.usedcardboardboxes.com/?id=" . encrypt_url(get_loop_box_id($dt_view_row_inv["ID"])) . "' target='_blank' >View Item</a></td>";
											$tmpTDstr =  $tmpTDstr . "<td bgColorrepl >";
											if ($actual_po < 0) {
										...
										if (isset($_REQUEST["display_view"]) && $_REQUEST["display_view"] == "2") {
											$tmpTDstr = "<tr  >";
											$tmpTDstr =  $tmpTDstr . "<td bgColorrepl><a href='https://b2bquote.usedcardboardboxes.com/?id=" . get_loop_box_id($dt_view_row_inv["ID"]) . "' target='_blank' >View Item</a></td>";
											$tmpTDstr =  $tmpTDstr . "<td bgColorrepl>";
											if ($actual_po < 0) {
												$tmpTDstr =  $tmpTDstr . "<font color='blue'>" . number_format($actual_po, 0) . $pallet_val_afterpo . $preorder_txt2 . "</td>";
											} else {
