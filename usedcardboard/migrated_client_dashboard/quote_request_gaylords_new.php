...
<?php } else {
	...
	// Added by Mooneem Jul-13-12 to Bring the green thread at top	
		...
		if ($_REQUEST["load_all"] == 1){
			...
			$dk = "SELECT *, inventory.id AS I, inventory.lengthInch AS L, inventory.widthInch AS W, inventory.depthInch AS D, inventory.notes AS N, inventory.date AS DT, inventory.vendor AS V FROM inventory WHERE (inventory.box_type = 'Gaylord' or inventory.box_type = 'Loop' or inventory.box_type = 'GaylordUCB' or inventory.box_type = 'PresoldGaylord') and inventory.Active LIKE 'A' ORDER BY inventory.availability DESC";
		}else{
			if ($_REQUEST["display-allrec"] == 1) {
				$dk = "SELECT *, inventory.id AS I, inventory.lengthInch AS L, inventory.widthInch AS W, inventory.depthInch AS D, inventory.notes AS N, inventory.date AS DT, inventory.vendor AS V FROM inventory WHERE (inventory.box_type = 'Gaylord' or inventory.box_type = 'Loop' or inventory.box_type = 'GaylordUCB' or inventory.box_type = 'PresoldGaylord') and (b2b_status=1.0 or b2b_status=1.1 or b2b_status=1.2) AND inventory.Active LIKE 'A' ORDER BY inventory.availability DESC";
			}
			elseif($_REQUEST["display-allrec"] == 2){
				$dk = "SELECT *, inventory.id AS I, inventory.lengthInch AS L, inventory.widthInch AS W, inventory.depthInch AS D, inventory.notes AS N, inventory.date AS DT, inventory.vendor AS V FROM inventory WHERE (inventory.box_type = 'Gaylord' or inventory.box_type = 'Loop' or inventory.box_type = 'GaylordUCB' or inventory.box_type = 'PresoldGaylord') and (b2b_status=1.0 or b2b_status=1.1 or b2b_status=1.2 or b2b_status=2.0 or b2b_status=2.1 or b2b_status=2.2) AND inventory.Active LIKE 'A' ORDER BY inventory.availability DESC";
			}
	...
	//-------------------------------------------------------------------------------------
	  ...
	  $dt_view_qry = "";
		...
		$tmpwarenm = ""; $tmp_noofpallet = 0; $ware_house_boxdraw = ""; $count = 0; $tipcount_match_str = "";
		while ($dt_view_row = array_shift($dt_view_res)) {
			...
			if ($vents_okay=="" && (int) $dt_view_row_inv["vents_yes"] == 0) {
				...
				/*if ($dt_view_row_inv["location"] != "" )
				{
					$tipStr = $tipStr . "Location: " . $dt_view_row_inv["location"] . "<br>";
				}*/
			...
			if (($dt_view_row["actual"] != 0) OR ($dt_view_row["actual"] - $sales_order_qty !=0 ))
			{
				...
				if (isset($_REQUEST["display_view"]) && $_REQUEST["display_view"]=="1") {
					...
					$MGArray[] = array('arrorder' => $mileage, 'arrdet' => $tmpTDstr, 'box_urgent' => $dt_view_row["box_urgent"]);
				  }
				  }
				}	
			}
