...
if(isset($_REQUEST['txtshippingaddEmail']) && !empty($_REQUEST['txtshippingaddEmail'])){
	...
	$product_loopboxid = 0; $productTotal = 0; $productQnt =0;
	$getSessionDt = db_query("SELECT product_loopboxid, product_total, product_qty FROM b2becommerce_order_item WHERE session_id = '".$sessionId."' and product_loopboxid = '" . $ProductLoopId ."'", db() );
	while ($rowContactInfo = array_shift($getSessionDt)) {
		...
		$productQnt = str_replace(",", "", $rowContactInfo['product_qty']);
	}
	...
	$qryUpdt = "UPDATE b2becommerce_order_item SET quote_name = '".$orderQuote['quoteName']."', quote_qty = '". $orderQuote['quoteQty'] ."', quote_unit_price = '". $orderQuote['quoteUnitPr'] ."', quote_total = '". $orderQuote['quoteTotal'] ."', quote_rate = '". $orderQuote['quoteRate'] ."' where session_id = '".$sessionId."' and product_loopboxid = '" . $ProductLoopId ."'";
	//echo $qryUpdt;
	$resUpdt = db_query($qryUpdt, db());
