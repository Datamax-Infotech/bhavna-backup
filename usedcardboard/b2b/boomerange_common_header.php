          ...
          <?php 
              if(isset($_REQUEST['uid']) && $_REQUEST['uid']!= "" ){ 
                  ...
                  ?>
                    <?php 
                    db();
                    $select_user = db_query("SELECT user_name FROM boomerang_usermaster WHERE loginid = '".$loginid."'");
                    if(tep_db_num_rows($select_user)>0){
                      $user = array_shift($select_user);
                      echo '<a href="'.$boomerang_url.'user_profile.php" class="mr-4 company_text_header">'.$user['user_name'].'</a>';
                    }
                  }
                  else{
