...
if(isset($_REQUEST['txtCntInfoEmail']) && !empty($_REQUEST['txtCntInfoEmail'])){
	...
	$session_login_id  			= $_REQUEST['session_login_id'];
	...
	if($createAccVal == 2 ){
		...
		if($rec_found == "no"){
			$rec_found = "no";
			db_query("INSERT INTO b2becommerce_user_master(first_name, last_name, email, password, phone) VALUES ('".$orderData['cntInfoFNm']."', '".$orderData['cntInfoLNm']."', '".$orderData['cntInfoEmail']."', '".$txtCrAccPass."', '".$orderData['cntInfoPhn']."' )", db() );
	...
	if($rec_found == "no"){
		...
		product_unitprice, product_total, contact_firstname, contact_lastname, contact_company, contact_email, contact_phone, order_date, session_login_id ) 
		VALUES('".$userMasterId."', '".$sessionId."', '".$productLoopId."', '".$proNameid."', '".str_replace("'", "\'" ,$productName)."', '".$productQnt."', '".$productUnitPr."', '".$productTotal."', '".str_replace("'", "\'" ,$orderData['cntInfoFNm'])."', '".str_replace("'", "\'" ,$orderData['cntInfoLNm'])."', 
		'".str_replace("'", "\'" ,$orderData['cntInfoCompny'])."', '".str_replace("'", "\'" ,$orderData['cntInfoEmail'])."', '".str_replace("'", "\'" ,$orderData['cntInfoPhn'])."', '".date('YmdHis')."', '".$session_login_id."' )", db());
		$insertedId = tep_db_insert_id();
		$responce = $insertedId;
	}else{
		$responce = $productLoopId;
		$qryOrderDt = db_query("Update b2becommerce_order_item set session_login_id = '".$session_login_id."', user_master_id = '".$userMasterId."', product_name_id = '". $proNameid ."', product_name = '".str_replace("'", "\'" ,$productName)."', 
		...
		contact_email = '".str_replace("'", "\'" ,$orderData['cntInfoEmail'])."', contact_phone = '".str_replace("'", "\'" ,$orderData['cntInfoPhn'])."' where session_id = '".$sessionId."' and product_loopboxid = '" . $productLoopId ."'" , db());
	}
