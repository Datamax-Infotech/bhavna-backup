...
$resUpdt = db_query($qryUpdt, db());*/
$qry = "Select type FROM loop_boxes WHERE id =" . $ProductLoopId;		
$res = db_query($qry, db() );		
$myrow = array_shift($res);
$id2 = $myrow["b2b_id"];	
$qryb2b = "Select * FROM inventory WHERE id =" . $id2;		
$resb2b = db_query($qryb2b, db_b2b() );		
$rowb2b = array_shift($resb2b);
...
if (in_array(strtolower($testvalue), array_map('strtolower', array("Gaylord", "GaylordUCB" ))))
	{ 
	$boxtypename = "Gaylord"; 
} elseif (in_array(strtolower($testvalue), array_map('strtolower', array("Medium", "Large", "Xlarge", "Box", "Boxnonucb", "Presold" ))))
	{ 
	$boxtypename = "Shipping Boxe"; 
} elseif (in_array(strtolower($testvalue), array_map('strtolower', array("SupersackUCB", "SupersacknonUCB" ))))
	{ 
	$boxtypename = "Supersack"; 
} elseif (in_array(strtolower($testvalue), array_map('strtolower', array("DrumBarrelUCB", "DrumBarrelnonUCB" ))))
	{ 
	$boxtypename = "DrumBarrelIBC"; 
} elseif (in_array(strtolower($testvalue), array_map('strtolower', array("PalletsUCB", "PalletsnonUCB"))))
	{ 
	$boxtypename = "Pallet"; 
} elseif (in_array(strtolower($testvalue), array_map('strtolower', array("Recycling"))))
	{ 
	...
	<div class="sections">
		<div class="new_container no-top-padding">
			...
			<div class="section-top-margin_1">
				<h1 class="section-title">Buy <?=$type;?></h1>
					...
					<form class="edit_checkout animate-floating-labels payfrm" data-payment-form="" action="/34391261315/checkouts/f44162a62f89b8d6b8dcf08c6a68fbb6" accept-charset="UTF-8" method="post">
						<div class="frmsection">
							...
							<div class="section__content">
								<fieldset class="content-box">
									<?
									//*****TO GET CC DETAILS IF ALREADY AVAILABLE AND USER LOGIN
									...
									if(!empty($rowUserDt['companyid'])){
										$getCompLoopId = db_query("SELECT loopid FROM companyInfo WHERE ID = ".$rowUserDt['companyid'], db_b2b());
										$rowCompLoopId = array_shift($getCompLoopId);
									}
									$rec_id = '21482';   //Assumption
									if(!empty($rec_id) ){ 
										$dt_view_qry = "SELECT * from loop_transaction_buyer WHERE id = '" .  $rec_id. "' AND po_file != ''";
										$result = db_query($dt_view_qry, db());
						...
						<ol class="name-values" style="width: 100%;">
								...
								<li>
									...
									<span id="Span1">
										...
										<br />
										<span id="Span1" class="caltxt">
										<?php echo find_distance($orderData["ProductLoopId"], $orderData['shippingaddZip']);?>
										mi from item origin</span>
									</span>
								</li>
								<li>
