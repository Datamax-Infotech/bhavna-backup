...
function productinfo(selectedval){  
	...
	function formvalidate(){
        var box_qtyval = document.getElementById('box_qty').value;
        
		if(box_qtyval == "Please Select" || box_qtyval == ""){
            alert("Please select Truckload quantity you want to order.");
            return false;
        } else {
		...
		$("#box_qty").select2({
			...
			templateResult: formatOptions
					...
					if (selectedOption.text() != ""){
						var quantity = selectedOption.text().split(' ')[0];
						var price = selectedOption.text().split(' ')[1];
						var rendered = '<div class="left">' + quantity + '</div><div class="right">' + price + '</div>';
						$(this).siblings('.select2').find('.select2-selection__rendered').html(rendered);
					}
				});

		$("#box_qty").trigger('change');

		function formatOptions(option) {
			if (!option.id) {
				return option.text;
			}
			var quantity = option.text.split(' ')[0];
			var price = option.text.split(' ')[1];
			return $('<span>' + quantity + '<span style="float:right;">' + price + '</span></span>');
		}
...
<style>
	...
	.select2-container--default .select2-results__option--highlighted, .select2-container--default .select2-results__option--selected.select2-container--default .select2-results__option.select2-results__option--active, .select2-container--default .select2-results__option:hover ,select2-container--default .select2-results__option.select2-results__option--active:hover {
	...
	.select2-container--default .select2-selection--single .select2-selection__rendered .left {
		text-align: left;
	}

	.select2-container--default .select2-selection--single .select2-selection__rendered .right {
		text-align: right;
	}	
</style>
</head>
...
if ($row["box_warehouse_id"] == 238){
	$after_actual_inventory = $row["after_po"];

	$b2b_quantity = $row["boxes_per_trailer"];
	
	$no_of_loads = 0;
	db();
	$sel_qry = "SELECT load_available_date, trans_rec_id from loop_next_load_available_history where inv_loop_id =".$id." and inactive_delete_flg = 0 
	and (load_available_date <= '" . date("Y-m-d", strtotime("+4 week")) . "') order by load_available_date";
	$sel_res = db_query($sel_qry );
	while ($res_box_row = array_shift($sel_res)) {
		if ($res_box_row["trans_rec_id"] == 0 ){ 
	...
	if($row["ship_ltl"] == 1 && $after_actual_inventory > $load_quantity){
	...
	$dt_so_item .= " where location_warehouse_id = '" . $row["box_warehouse_id"] . "' and box_id = " . $id . " and loop_transaction_buyer.bol_create = 0 and loop_transaction_buyer.ignore = 0 ";
	...
	$actual_po = $row["actual_qty_calculated"] - $sales_order_qty; 
...
$quantity_availableValue_new1 = getQtyAvNow($id, $row["box_warehouse_id"], $lead_time, $row["ship_ltl"], $rowb2b["after_actual_inventory"],  $row["boxes_per_trailer"], $row["actual_qty_calculated"], $sales_order_qty );
...
$dt_view_qry = "SELECT afterpo from tmp_inventory_list_set2 where trans_id = '" . $id . "' order by warehouse, type_ofbox, Description";
...
//$getEstimatedNextLoad = getEstimatedNextLoad_New($row["id"], $row["box_warehouse_id"], $next_load_available_date, $lead_time, $lead_time, $txt_after_po_new, $row["boxes_per_trailer"], $row['expected_loads_per_mo'], $status_key, 'no');
...
-------------------------------------------------------------------------------*/
?>
	<div class="new_section new-section-margin pt-0">
			...
			<div class="content-div content-padding">
				<div class="left_qty">
					<table class="qty-table-top">
						<tr>
							<td><strong>Qty Avail Now </strong> <div class="tooltip"><i class="fa fa-info-circle" aria-hidden="true"></i><span class="tooltiptext">Quantity we have available for sale now.</span></div></td>
							<td>
								<?php
									echo number_format($quantity_availableValue_new);
								?>
							</td>
						</tr>
						...
						<tr>
							<td><strong>% of FTL </strong> <div class="tooltip"><i class="fa fa-info-circle" aria-hidden="true"></i><span class="tooltiptext">FTL = Full Truckload, which is a 53' dry van trailer delivery of boxes. This is the % of a full truckload that is available for purchase.</span></div></td>
							<td>
								<?php
									$percent_per_load = 0;
									$ftl_qty = $rowb2b["quantity"];
									if ($ftl_qty > 0){
										$percent_per_load = number_format((str_replace(",", "" , $quantity_availableValue_new) / str_replace(",", "" , $ftl_qty)),2);
									}
								
									echo $percent_per_load;
								?>
							</td>
						</tr>

						<tr>
							<td><strong>Lead Time </strong> <div class="tooltip"><i class="fa fa-info-circle" aria-hidden="true"></i><span class="tooltiptext">How long it will take to ship any quantity we have available now (Qty Avail Now).</span></div></td>
							<td>
								<?php
									$lead_time = $rowb2b["lead_time"];
									$lead_timeView = "";
									if ($quantity_availableValue_new == 0){
										$lead_timeView = "N/A";
									}else{
									
										if($lead_time == 1 || $lead_time == 0){
											$lead_timeView= "1 Day";
										}else{
											if($lead_time < 7){
												$lead_timeView= $lead_time . " Days";
											}
										}
										
										if($lead_time >= 7){
											$lead_timeWeek = round($lead_time/7);
											if($lead_timeWeek == 1){
												$lead_timeView= $lead_timeWeek . " Week";
											}else{
												$lead_timeView= $lead_timeWeek . " Weeks";
											}
										}
									}
									...
									$estimated_next_load = $lead_timeView;
						...
						<tr>
							<td><strong>FTL Qty </strong> <div class="tooltip"><i class="fa fa-info-circle" aria-hidden="true"></i><span class="tooltiptext">Full 53' truckload (FTL) quantity. This is how UCB ships 95% of our orders which provides cheapest price points.</span></div></td>
							<td>
								<?php
									echo number_format($rowb2b["quantity"]);
								?>
							</td>
						</tr>
				        ...
				        <tr>
							...
							<span class="tooltiptext">Calculate the shipping cost of a full 53' truckload (FTL) to you.</span></div></td>
							<td >
					...
					<table class="qty-table" style="table-layout: auto;">			
						<tr>
						...
						<tr>
							...
							<span>
								...
								$full_qty_val = str_replace(",", "" , number_format(trim($row["boxes_per_trailer"])));
								$full_qty_price = "$" . number_format($full_qty_val * $fullUnitPr,2);
								?>

								<?
									$pallet_qty = (int)($row["bpallet_qty"]);
									$qty_available = floor($quantity_availableValue_new);
									//echo "pallet_qty - " . $pallet_qty . " " . $qty_available . "<br>";
								?>
									<select id="box_qty" name="box_qty" class="js-states form-control" onchange="showbox_price()" >
									<?php  
									if($row["ship_ltl"] == 0){	
										echo "<option value='1|" . $full_qty_val  . "|" . $fullUnitPr . "'>" . $full_qty_val ." $" . number_format($fullUnitPr,2) ." </option>";
									}else{
										$full_qty_price = "";
										echo "<option value=''></option>";
										$pallet_qty = $row["bpallet_qty"];
										if ($pallet_qty > 0){
											$qty_available = floor($quantity_availableValue_new);
											$qty_half = $row["boxes_per_trailer"]/2;
											$qty_qtr = $row["boxes_per_trailer"]/4;
												...
												//$palletUnitPr 	= $fullUnitPr + 3.00;
											$productQntype = "1";
											for ($qty_available_cnt = $pallet_qty; $qty_available_cnt <= $qty_available; $qty_available_cnt = (int)($qty_available_cnt) + (int)($pallet_qty))
											{
												if (($qty_available_cnt < $full_qty_val) && ($qty_available_cnt >= $qty_half)){
													$price = $halfUnitPr;
													$productQntype = "2";
												}
												if (($qty_available_cnt < $qty_half) && ($qty_available_cnt >= $qty_qtr)){
													$price = $qrtrUnitPr;
													$productQntype = "3";
												}
												if (($qty_available_cnt < $qty_qtr) && ($qty_available_cnt > $pallet_qty)){
													$price = $palletUnitPr;
													$productQntype = "4";
												}
												if ($qty_available_cnt > $full_qty_val){
													$price = $fullUnitPr;
													$productQntype = "1";
												}

												if ($qty_available_cnt == $pallet_qty){
													$price = $palletUnitPr;
													$productQntype = "4";
												}

												echo "<option value='" . $productQntype . "|" . $qty_available_cnt  . "|" . $price . "'>" . $qty_available_cnt ." $" . number_format($price,2) . " </option>";
												//echo "qty_available_cnt = " . $qty_available_cnt . "<br>";
											}
										}
										//echo "full_qty_val = " . $full_qty_val . " | " . $qty_available . "<br>";
										if ($full_qty_val <= $qty_available){
										//	echo "<option value='1|" . $full_qty_val . "|" . $fullUnitPr . "'>" . $full_qty_val ." $" . number_format($fullUnitPr,2) ." </option>";
										}
									}
									?>									
							...
							<td>
								<span id="box_total_price" style="font-weight: bold;" ><? echo number_format($full_qty_price); ?></span>
							</td>
							...
							<td >
								...
								<button type="button" onclick="javascript: formvalidate();" class="button_slide slide_right" style="white-space: nowrap; margin-top:0px;" data-testid="order-button">Proceed to Checkout</button>
							</td>
						 </tr>
					</table>	
				    </form>
					...
					<div class="qty-div">
						<table class="qty-table" id="productInfo">
							...
							?>
							<tr id="productinfo_1" onclick="javascript: productinfo(1);">
								<td align="left">
				...
				<?
					...
					$shape = ""; $bottom = ""; $top=""; $bottom_heading_txt = ""; $top_heading_txt ="";
					...
					{
						$top_1 = "None";
						$top_heading_txt = "No Top";
					}
					if ($b2b_top_partial == 1)
					{
						$top_2 = "Partial Flap";
						$top_heading_txt = "Partial Top";
					}
					if ($b2b_top_full == 1)
					{
						$top_3 = "Full Flap";
						$top_heading_txt = "Full Top";
					}
					if ($b2b_top_hinged == 1)
					{
						$top_4 = "Hinged Lid";
						$top_heading_txt = "Hinged Lid";
					}
					if ($b2b_top_remove == 1)
					{
						$top_5 = "Removable Lid";
						$top_heading_txt = "Removable Top";
					}
					if ($b2b_top_open == 1)
					{
						$top_6 = "Open Top";
						$top_heading_txt = "Open Top";
					}
					if ($b2b_top_spout == 1)
					{
						$top_7 = "Spout Top";
						$top_heading_txt = "Spout Top";
					}
					if ($b2b_top_duffle == 1)
					{
						$top_8 = "Duffle Top";
						$top_heading_txt = "Duffle Top";
					}

					if (($b2b_top_partial + $b2b_top_full + $b2b_top_remove + $b2b_top_open + $b2b_top_spout + $b2b_top_duffle) > 1)
					{
						$top_heading_txt = "Mixed Top";
					}

					if ($b2b_bottom_no == 1)
					{
						$bottom_1 = "None";
						$bottom_heading_txt = "No Bottom";
					}
					if ($b2b_bottom_partial == 1)
					{
						$bottom_2 = "Partial Flap Without Slip Sheet";
						$bottom_heading_txt = "Partial Bottom";
					}
					if ($b2b_bottom_partialsheet == 1)
					{
						$bottom_3 = "Partial Flap With Slip Sheet";
						$bottom_heading_txt = "Slipsheet Bottom";
					}
					if ($b2b_bottom_fullflap == 1)
					{
						$bottom_4 = "Full Flap";
						$bottom_heading_txt = "Full Bottom";
					}
					if ($b2b_bottom_interlocking == 1)
					{
						$bottom_5 = "Interlocking Flaps";
						$bottom_heading_txt = "Interlocking Flaps";
					}
					if ($b2b_bottom_tray == 1)
					{
						$bottom_6 = "Tray";
						$bottom_heading_txt = "Tray Bottom";
					}
					if ($b2b_bottom_spout == 1)
					{
						$bottom_7 = "Spout Bottom";
						$bottom_heading_txt = "Spout Bottom";
					}
					if ($b2b_bottom_flat == 1)
					{
						$bottom_8 = "Flat Bottom";
						$bottom_heading_txt = "Flat Bottom";
					}
					if ($b2b_bottom_spiked == 1)
					{
						$bottom_9 = "Flat Spiked Bottom";
						$bottom_heading_txt = "Flat Spiked Bottom";
					}
					
					if (($b2b_bottom_partial + $b2b_bottom_partialsheet + $b2b_bottom_partialsheet + $b2b_bottom_fullflap + $b2b_bottom_tray + $b2b_bottom_spout + $b2b_bottom_flat + $b2b_bottom_spiked) > 1)
					{
						$bottom_heading_txt = "Mixed Bottom";
					}
					...
					$dimension = ""; $wall_str = ""; $idTitle_new = ""; $idTitle_gaylord = ""; $idTitle_shipping = "";
					if($row["uniform_mixed_load"] == "Mixed"){					
						...
						if ($bheight_min == $bheight_max){
							...
							$idTitle_gaylord = $bheight_min . "\"";
						}else{
							$dimension .= $bheight_min . ' - '. $bheight_max . '"';
							$idTitle_gaylord = $bheight_min . ' - '. $bheight_max . "\"";
						}											
						
						$idTitle_shipping = $dimension;
						...
						$idTitle_gaylord .= " " . $wall_str . " " . $bottom_heading_txt;
						...
						if ($depth != "") {$depth_str = $depth . "\""; } else {$depth_str = "";}
						$idTitle_gaylord = $depth_str . " " . $wall_str . " " . $bottom_heading_txt;

					}
					
					if (in_array(strtolower(trim($box_type)), array_map('strtolower', array("Gaylord", "GaylordUCB", "Loop", "PresoldGaylord" )))){ 
						$idTitle_new = $idTitle_gaylord;
					}elseif (in_array(strtolower(trim($box_type)), array_map('strtolower', array("Medium", "Large", "Xlarge", "LoopShipping", "Box", "Boxnonucb", "Presold" )))){ 
						$idTitle_new = $idTitle_shipping;
					}else{
						$idTitle_new = $idTitle;
					}
					
					$_SESSION['idTitle_new'] = $idTitle_new;
				?>				
				
				<div class="right-products">
					<div class="productinfo-section">
					<div class="productinfo-main product-box-shadow">
						<h2 class="prod-title"><? echo $idTitle_new; ?></h2>
						...
						<div class="prod-info">
							...
							<ol class="name-value" style="width: 100%;">
								<li>
									<label for="Span2">ID</label>
									<span id="Span2"><? echo $invId;?></span><br><br>
								</li>
								<li>                    
...
?>
	<script>
		...
		function showbox_price(){
			var box_qty_val = document.getElementById("box_qty").value;
			if (box_qty_val != ""){
				box_qtyvalue = box_qty_val.split("|")[0];
				box_qty = box_qty_val.split("|")[1];
				box_qty_unit_price = box_qty_val.split("|")[2];
				
				box_final_price = box_qty_unit_price * box_qty;
				if (box_final_price){
					box_final_price = box_final_price.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
				}
				document.getElementById("box_total_price").innerHTML = "$" + box_final_price;
				
				document.getElementById("productQntypeid").value = box_qtyvalue;
				if (box_qtyvalue == 1){
					document.getElementById("productQntype").value = "Full";
				}else if (box_qtyvalue == 2){
					document.getElementById("productQntype").value = "Half";
				}else if (box_qtyvalue == 3){
					document.getElementById("productQntype").value = "Quarter";
				}else if (box_qtyvalue == 4){
					document.getElementById("productQntype").value = "1 Pallet";
				}
				document.getElementById("productQnt").value = box_qty;
				document.getElementById("productQntprice").value = box_qty_unit_price;
				document.getElementById("productTotal").value = box_final_price;
