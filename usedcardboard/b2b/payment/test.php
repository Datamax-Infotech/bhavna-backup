...
<div align="center">
	...
	<form name="frmpayform" id="frmpayform" action="">
		<!-- <input type="text"  name="selected_id" id="selected_id" value="<? echo $_REQUEST["selected_id"];?>" />
		<input type="text"  name="rec_id" id="rec_id" value="<? echo $_REQUEST["rec_id"];?>" />
		<input type="text"  name="page_name" id="page_name" value="<? echo $_REQUEST["page_name"];?>" /> -->
		...
		<table cellspacing="1" cellspacing="1" align="center" border="0" >
			...
			<tr>
				...
				<td >
					<select name="payment_type" id="payment_type" onchange="payment_type_chg()">
						<option value="authCaptureTransaction" <? if ($_REQUEST["payment_type"] == "authCaptureTransaction") { echo " selected ";}?>>Authorization and Capture</option>
						<option value="authOnlyTransaction" <? if ($_REQUEST["payment_type"] == "authOnlyTransaction") { echo " selected ";}?> >Authorization Only</option>
						<option value="priorAuthCaptureTransaction" <? if ($_REQUEST["payment_type"] == "priorAuthCaptureTransaction") { echo " selected ";}?>>Prior Authorization Capture</option>
						<!-- <option value="voidTransaction" <? if ($_REQUEST["payment_type"] == "voidTransaction") { echo " selected ";}?> >Void</option>
						<option value="refundTransaction" <? if ($_REQUEST["payment_type"] == "refundTransaction") { echo " selected ";}?> >Refund</option> -->
					</select>
				</td>
			</tr>
			...
			<tr>
				<td ><font size="2">Amount</font></td>
				<td >
					<input name="trans_amount" id="trans_amount" value="<? echo $_REQUEST["trans_amount"];?>"/>
				</td>
			</tr>
			  ...
			  <tr>
				<td>
					...
					<div class="container-fluid" style="width: 100%; margin: 0; padding:0">
						...
						<div >
							<form id="post_data" name="post_data" action="https://loops.usedcardboardboxes.com//viewCompany.php?ID=<? echo $_REQUEST["selected_id"];?>&show=transactions&warehouse_id=<? echo $warehouse_id;?>&rec_type=Supplier&proc=View&searchcrit=&id=<? echo $warehouse_id;?>&rec_id=<? echo $_REQUEST["rec_id"];?>&display=<? echo $_REQUEST["page_name"];?>&trans_type=<? echo $_REQUEST["payment_type"];?>" method="post" >
								...
								<input type="hidden" name="response_amt" id="response_amt" value="<? echo $po_poorderamount;?>" />
							...
							<form id="return_page" name="return_page" action="https://loops.usedcardboardboxes.com//viewCompany.php?ID=<? echo $_REQUEST["selected_id"];?>&show=transactions&warehouse_id=<? echo $warehouse_id;?>&rec_type=Supplier&proc=View&searchcrit=&id=<? echo $warehouse_id;?>&rec_id=<? echo $_REQUEST["rec_id"];?>&display=<? echo $_REQUEST["page_name"];?>&trans_type=<? echo $_REQUEST["payment_type"];?>" method="post" >
							</form>
						</div>
