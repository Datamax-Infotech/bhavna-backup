    ...
    <div class="row">
      <div class="col-3">
        <div class="nav flex-column nav-pills my-account-tab" id="my-account-tab" role="tablist" aria-orientation="vertical">
          ...
          <button class="nav-link" id="manage-address-tab" data-toggle="pill" data-target="#manage-address" role="tab" aria-controls="manage-address" aria-selected="false">Manage Address</button>
        </div>
      </div>
      <div class="col-9">
        <div class="tab-content" id="my-account-tabContent">
                ...
                <div class="col-md-8">
                  <form id="edit_profile_form">
                    ...
                    <div class="form-group">
                      ...
                      <select multiple name="companies[]" id="all_companies" class="form-control form-control-sm">
                        <option>Select Companies</option>
                        <?php
                        db();
                        $select_comp = db_query("SELECT * from boomerang_user_companies where user_id = '" . $loginid . "'");
                        $selected_companies = array();
                        while ($row = array_shift($select_comp)) {
                          $selected_companies[] = $row['company_id']; // Assuming 'company_id' is the column that matches with 'ID' in 'companyInfo' table
                        }
                        db();
                        ...
                        while ($row = array_shift($select_sales_comp)) {
                          $selected = in_array($row['ID'], $selected_companies) ? 'selected' : '';
                ...
                <div class="col-md-4">
                  ...
                  <p class="mt-2">
                    ...
                    if (count($selected_companies) > 0) {
                      $comp_ids = implode(',', $selected_companies);
                      db_b2b();
                      $select_sales_comp = db_query("SELECT ID, company FROM companyInfo where ID IN ($comp_ids) ORDER BY company");
                      while ($row = array_shift($select_sales_comp)) {
                        echo $row['company'] . "<br>";
                      }
                    } else {
                      echo "<p class='text-danger'>You dont have any company Selected!</p>";
                    }
                    ?>
                  </p>
                </div>
          ...
          <div class="tab-pane fade" id="manage-address" role="tabpanel" aria-labelledby="manage-address-tab">
            ...
            <?php
            db();
            $address_qry = db_query("SELECT * FROM boomerang_user_addresses WHERE user_id = '" . $loginid . "'");
              ...
              while ($address = array_shift($address_qry)) { ?>
                <div class="card mt-3 <?php echo $address['mark_default'] == 1 ? "border-success" : ""; ?>">
                  <div class="card-body">
                    <div class="row">
                      <div class="col-md-8">
                        <p><b>Name:</b> <?php echo $address['name'] ?></p>
        ...
        <div class="modal-body">
          <div class="form-row">
            <div class="form-group col-md-6">
              <label>Name</label>
              <input id="add_name" type="text" class="form-control form-control-sm" name="name" placeholder="Enter Name">
              <span class="form_error d-none" id="add_name_error">Name Can't Be Blank</span>
            </div>
            ...
            <div class="form-group col-md-12">
              ...
              <input id="add_addressline2" type="text" class="form-control form-control-sm" name="addressline2" placeholder="Enter addressline2">
            </div>
            <div class="form-group col-md-6">
              <label>City</label>
...
<script>
  ...
  $(document).ready(function() {
    ...
    $("#add_address_form").submit(function() {
      ...
      if (add_name == '') {
        $('#add_name_error').removeClass('d-none');
        flag = false;
      } else {
        $('#add_name_error').addClass('d-none');
      }
