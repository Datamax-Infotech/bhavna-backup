...
Date           		By         	   	Description
=============================================================================================================
19-April-22 		Bhavna		Show list of inventory which is to be hide.
...
$get_boxes_query = db_query("SELECT * FROM loop_boxes WHERE type = 'Gaylord' OR type = 'GaylordUCB' OR type = 'Loop'OR type = 'PresoldGaylord' OR type = 'Medium' OR type = 'Large' OR type = 'Xlarge' OR type = 'LoopShipping' OR type = 'Box' OR type = 'Boxnonucb' OR type = 'Presold' OR type = 'SupersackUCB' OR type = 'SupersacknonUCB' OR type = 'PalletsUCB' OR type = 'PalletsnonUCB' ORDER BY b2b_id limit 100", db());	
while ($boxes = array_shift($get_boxes_query)) 	 {	
	?>							
	<tr <?$c = 0; if ($c == 1) {echo " bgcolor=lightgrey";$c = 0;} else { $c = 1; }?>>									
		<td valign="top" class="style7">			
			<div id='updbox_action_div<?=$boxes["id"];?>'>
			<?
			$checked_sql = "SELECT * FROM boomerang_inventory_hide_items WHERE hide_b2bid = '" . $boxes["b2b_id"] . "' AND hideItems = 1 AND user_id = '" . $user_id . "'"; 
			$result_cnt = db_query($checked_sql,db() );
			$number = tep_db_num_rows($result_cnt);
			//$boxes["id"];
			if ($number > 0) {
				?>
				<input type="button" name="btnremove" value="Unhide" onclick="Remove_boxes_hide(<?php echo $boxes['b2b_id'];?>,<?php echo $user_id ?>)">
				<? 
			} else {		
				?>	
				<input type="button" name="btnadd" value="Hide" onclick="Add_boxes_hide(<?php echo $boxes['b2b_id'];?>,<?php echo $user_id ?>)"> 
				<? 
			} ?>	
			</div>
		</td>							
		...
		<?
		$vender_nm = ""; $b2b_location_city = ""; $b2b_location_st = ""; $b2b_location_zip = ""; 	
