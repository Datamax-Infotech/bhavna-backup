...
?>
	...
	<tr class="display_maintitle">
			...
			<td>Status&nbsp;
				<a href="javascript:void();" onclick="displarepsorteddata('<?=$comp_sel;?>','<?=$vendors_dd;?>','<?=$ddMadePayment;?>', '<?=$date_from;?>', '<?=$date_to;?>', '<?=$receivables;?>', '<?=$payables;?>', 18, 1);" ><img src="images/sort_asc.png" width="5px" height="10px"></a>&nbsp;<a href="javascript:void();" onclick="displarepsorteddata('<?=$comp_sel;?>','<?=$vendors_dd;?>','<?=$ddMadePayment;?>', '<?=$date_from;?>', '<?=$date_to;?>', '<?=$receivables;?>', '<?=$payables;?>', 18, 2);" ><img src="images/sort_desc.png"  width="5px" height="10px"></a>
			</td>
		...
		if (isset($_REQUEST["vendors_dd"])) {
			if ($_REQUEST["vendors_dd"] != "All" && $_REQUEST["vendors_dd"] != "") {
				$vendorsQry = "Select *, water_vendors.id as vid from water_transaction inner join water_vendors on water_transaction.vendor_id=water_vendors.id inner join water_vendors_receivable_contact on water_transaction.vendor_id=water_vendors_receivable_contact.water_vendor_id where make_receive_payment = 1 and vendor_id = '".$_REQUEST["vendors_dd"]."' group by vendor_id";
				//$Vwhere_condition = " and vendor_id = " . $_REQUEST["vendors_dd"] ;
			
			}else{
				$vendorsQry = "
				Select *, water_vendors.id as vid from water_transaction inner join water_vendors on water_transaction.vendor_id=water_vendors.id inner join water_vendors_receivable_contact on water_transaction.vendor_id=water_vendors_receivable_contact.water_vendor_id where make_receive_payment = 1 group by vendor_id";
		...
		if (isset($_REQUEST["columnno"])) {
			...
			if ($_REQUEST["columnno"] == "2" && $_REQUEST["sortflg"] == "1") {
				$vendorsQry .= " order by company_id asc";
				$vendorsQry_order = " order by company_id asc";
			}	
			if ($_REQUEST["columnno"] == "2" && $_REQUEST["sortflg"] == "2") {
				$vendorsQry .= " order by company_id desc";
				$vendorsQry_order = " order by company_id desc";
			}	
		...
		//while ($data_row = array_shift($v_res)) {
			...
			if (isset($_REQUEST["vendors_dd"])) {
				if ($_REQUEST["vendors_dd"] != "All" && $_REQUEST["vendors_dd"] != "") {
					...
					where make_receive_payment = 1 and vendor_id='". $_REQUEST["vendors_dd"] ."' ".$whrMadePayConditn." $swhere_condition group by company_id, 
					water_transaction.id having sum(amount) > 0 $vendorsQry_order";
				}else{
					$vendorQry = "Select *,loop_warehouse.id as warehouse_id,water_vendors_receivable_contact.id as receivable_id, sum(amount) as amt, loop_warehouse.company_name, loop_warehouse.b2bid, water_transaction.id as transid 
					from water_transaction inner join loop_warehouse on loop_warehouse.id = water_transaction.company_id 
					inner join water_vendors on water_transaction.vendor_id=water_vendors.id
			...
			?>	
					<tr>
						...
						<td class="display_table">
							C: <?= $rows["receivable_contact_name"] ?> <br>
							P: <?= $rows["receivable_main_phone"] ?> <br>
							E: <?= $rows["receivable_email"] ?>
						</td>
						...
						<td class="display_table">
							C: <?= $rows["company_contact"] ?> <br>
							P: <?= $rows["company_phone"] ?> <br>
							E: <?= $rows["company_email"] ?>
						</td>
						...
						<td class="display_table"><?= $rows["receivable_portal_link"] ?></td>
						<td class="display_table" id="paid_by_td-<?=$unique_count;?>"><?= $rows["paid_by"] ?></td>
						<td class="display_table" id="paid_date_td2-<?=$unique_count;?>"><?=$rows["paid_date"]!="" ? date('m/d/Y', strtotime($rows["paid_date"])) :""; ?></td>
						<td class="display_table" id="ar_status_td-<?=$unique_count;?>"><? if($rows['made_payment'] == 1 ) { echo 'Paid'; } else { echo 'Active'; } ?></td>
						<td class="display_table">
							<? //$rows["receivable_notes"];
							$vendor_id_comm=$rows["vendor_id"];
							$special_notes_count_qry=db_query("SELECT count(*) as total_notes from water_vendors_payable_contact where water_vendor_id='$vendor_id_comm' AND payable_notes!='' ",db()); 
							$special_notes_qry=db_query("SELECT payable_notes from water_vendors_payable_contact where water_vendor_id='$vendor_id_comm' AND payable_notes!='' ORDER BY created_on DESC limit 1 ",db());
							echo array_shift($special_notes_qry)['payable_notes'];
							if(array_shift($special_notes_count_qry)['total_notes'] > 1){?>
								<br><a style="cursor:pointer" id='show_all_btn_<?=$unique_count?>' onclick="javascript:show_all_notes('payable',<?=$vendor_id_comm?>,'<?=$unique_count;?>')" >All Notes</a>
							<? } ?>
						</td>
						<td class="display_table">
							<input type='button' id='agtempl<?= $unique_count;?>' name='send_tmpl' value='Select Template' onclick='open_template_vendors_AR(1,<?=$rows["warehouse_id"]?>,<?= $rows["transid"];?>,<?= $data_row["vendor_id"];?>, "<?= $unique_count;?>")'/>	
						</td>
						<td class="display_table">
										...
										<tr>
											...
											<td class="display_table">
												<select name="ar_status" id="ar_status">
													...
													?>
														<option value="<?=$lowerarvalue?>" <?=$arselectedValue?>><?=$value?></option>
													<? } ?>
												</select>
											</td>
											...
											<td class="display_table">
												...
												<input type="hidden" name="vendorpagename"  value="UCBZeroWaste_Vendors_AR.php">
		...
		if ($_REQUEST["columnno"] == "DONOTUSE3") {
			...
			foreach ($MGarray as $MGArraytmp) { 
				...
				<tr>
					...
					<td class="display_table"><?= ucfirst($MGArraytmp["ar_status"]) ?></td>
						...
						?>
							...
							<table>
								<tr>
									...
									<td class="display_table">Status:</td>
									<td class="display_table">
										<select name="ar_status-<?=$row;?>" id="ar_status">
											<?
											foreach($arStatus as $arKey => $value){
												$lowerarvalue = $arKey != 0 ? strtolower($value) : "";
												$arselectedValue = isset($MGArraytmp["ar_status"]) && $MGArraytmp["ar_status"] != '' && $MGArraytmp["ar_status"] == $lowerarvalue ? 'selected' : ''; 
											?>
												<option value="<?=$lowerarvalue?>" <?=$arselectedValue?>><?=$value?></option>
											<? } ?>
										</select>
