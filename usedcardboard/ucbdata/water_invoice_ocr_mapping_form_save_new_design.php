    ...
    for ($index = 1; $index <= count($consider_water_mapping_table_array); $index++) {
        ...
        for ($index2 = 0; $index2 < count($water_material_array); $index2++) {
            ...
            VALUES($template_id,$index,'" . $tbl_column_no . "','" . preg_replace("/'/", "\'", $tbl_column_text) . "','" . preg_replace("/'/", "\'", $table_mapping_field) . "')";
                ...
                $material_fee = $water_material_array[$index2];
                $water_material = 0;
                $add_fee = 0;
                if (strpos($material_fee, 'mat-') === 0) {
                    $water_material = substr($material_fee, 4); // Remove prefix to get the actual ID
                } else if (strpos($material_fee, 'fee-') === 0) {
                    $add_fee = substr($material_fee, 4);
                }
                ...
                $custom_header_flg = !empty($_POST['row_custom_header_array' . $index][$index2]) ? 1 : 0;
                $custom_header_value = $custom_header_flg ? $_POST['row_custom_header_array' . $index][$index2] : 0;
                ...
                $insert_table_data = "INSERT INTO water_ocr_mapping_item_list_table_data (`table_no`,`template_id`,`row_no`,`water_material`,`add_fee`,`line_item`,`group_text`,`start_position`,`end_position`,`field_data`,`coordinates`, ocr_selected_text, `custom_header`)
				VALUES($index,$template_id,$row_no,$water_material,$add_fee,$line_item,$group_text,$start_position, $end_position,'" . preg_replace("/'/", "\'", $table_mapping_field_data) . "','" . $field_coordinates_data . "','" . preg_replace("/'/", "\'", $ocr_selected_text) . "','" . $custom_header_flg . "')";
                db_query($insert_table_data);
                if ($custom_header_flg != 0) {
                    $insert_tbl_head_data = "INSERT INTO water_ocr_mapping_item_list_table_custom_header (`template_id`,`table_no`,`row_no`,`header_value`)
                    VALUES($template_id , $index , '" . $row_no . "' , '" . preg_replace("/'/", "\'", $custom_header_value) . "')";
                    db_query($insert_tbl_head_data);
                }
                if ($table_consideration == 1) {
                    if ($line_item == 1) {
                        if ($selected_index_for_material_fee != 111) {
                            ...
                            */
                            ...
                            if ($group_text == 1) {
                                ...
                                if ($water_material != "" && $water_material != 0) {
            ...
            if ($index2 == 0) {
               ...
               // $add_fee = $_POST['add_fee' . $index][$index2] == "" ? 0 : $_POST['add_fee' . $index][$index2];
                ...
                if ($water_list_item_data_id != "") {
                    ...
                    //echo "<br> Field data =".$water_list_item_data['field_data'] ." - " .$table_mapping_field_data;
                    if ($water_list_item_data['water_material'] != $water_material || $water_list_item_data['add_fee'] != $add_fee || $water_list_item_data['line_item'] != $line_item || $water_list_item_data['group_text'] != $group_text || $water_list_item_data['start_position'] != $start_position || $water_list_item_data['end_position'] != $end_position || $water_list_item_data['field_data'] != $table_mapping_field_data || $water_list_item_data['coordinates'] != $field_coordinates_data || $water_list_item_data['ocr_selected_text'] != $ocr_selected_text || $water_list_item_data['custom_header'] != $custom_header_flg) {
                       $update_table_data = "UPDATE water_ocr_mapping_item_list_table_data set water_material = $water_material, add_fee = $add_fee, line_item = $line_item, group_text = $group_text, start_position = $start_position, end_position = $end_position, field_data = '" . preg_replace("/'/", "\'", $table_mapping_field_data) . "', coordinates = '" . $field_coordinates_data . "', ocr_selected_text = '" . preg_replace("/'/", "\'", $ocr_selected_text) . "', custom_header = '" . $custom_header_flg . "' where id = $water_list_item_data_id";
                ...
                if ($custom_header_flg != 0) {
                    $select_custom_header = db_query("SELECT * FROM water_ocr_mapping_item_list_table_custom_header where template_id = $template_id and table_no = $index and row_no = $row_no");
                    if (tep_db_num_rows($select_custom_header) > 0) {
                        $update_tbl_head_data = "UPDATE water_ocr_mapping_item_list_table_custom_header set header_value = '" . preg_replace("/'/", "\'", $custom_header_value) . "' where template_id = $template_id and table_no = $index and row_no = $row_no";
                        db_query($update_tbl_head_data);
                    } else {
                        $insert_tbl_head_data = "INSERT INTO water_ocr_mapping_item_list_table_custom_header (`template_id`,`table_no`,`row_no`,`header_value`)
                        VALUES($template_id , $index , '" . $row_no . "' , '" . preg_replace("/'/", "\'", $custom_header_value) . "')";
                    ...
                    db_query("DELETE FROM water_ocr_mapping_item_list_table_custom_header where template_id = $template_id and table_no = $index and row_no = $row_no");
                }
                if ($table_consideration == 1) {
                    if ($line_item == 1) {
