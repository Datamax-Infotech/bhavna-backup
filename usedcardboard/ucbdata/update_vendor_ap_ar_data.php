...
if (isset($_REQUEST['edit_report']) && $_REQUEST['edit_report'] == "yes") {
	...
	if (isset($_REQUEST['paid_by']) && ($_REQUEST['paid_by'] != "") && ($_REQUEST['paid_by'] != " ")) {
		...
		$get_updated_data_sql = db_query("SELECT made_payment,paid_by,paid_date,payment_method, payment_proof_file,ar_status,vendor_payment_log_notes,last_edited FROM water_transaction WHERE invoice_number = '" . $_REQUEST['hdnInvcNo'] . "' AND vendor_id = '" . $_REQUEST['hdnvendrId'] . "' and id = '" . $_REQUEST['hdnWatrTrnstnId'] . "'");
	...
	$data_str = "";
	$log_notes_qry = db_query("SELECT wt.date, wt.message,le.initials from water_transaction_log_notes as wt JOIN loop_employees as le ON le.b2b_id = wt.employee_id  where wt.trans_id='$transid' ORDER BY date DESC", db());
