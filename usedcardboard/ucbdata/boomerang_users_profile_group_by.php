...
<div class="main_data_css">
	...
	<select multiple name="companies[]" id="all_companies" style="display:none; ">
		...
		<?php
		$select_sales_comp = db_query("SELECT ID, company FROM companyInfo where haveNeed = 'Have Boxes' && company!='' ORDER BY company", db());
	...
	<table class="table_boomerang_portal">
					...
					$row = array_shift($select_users);
							...
							echo "<tr id='userrowid_" . $row['loginid'] . "'>
									...
									<td>
										...
										<button type='button'><a href='boomerange_users_profile.php?user_id=".$row['loginid']."'>View</a></button>
									</td>	
									</tr>";
					?>
					</table>
		...
		if (tep_db_num_rows($select_user_companies) > 0) {
			while ($row1 = array_shift($select_user_companies)) {
				db_b2b();
				$company_data_qry = db_query("SELECT * FROM companyInfo where ID = '" . $row1['company_id'] . "'");
				$dt_view_res = array_shift($company_data_qry);
				$company_id = $dt_view_res['ID'];
				echo '<tr align="center"><td colspan="19" width="320px" align="left" bgcolor="#C1C1C1">'.$dt_view_res['company'].'</td>';
		?>
		...
		<tr>   
			<td colspan="6" >
				<form name="frmFavItems" method="post" action="boomerange_users_profile.php?ID=<?php echo $company_id;?>">
				<table width="100%" cellspacing="1" cellpadding="1"border="0" style="font-family:Arial, Helvetica, sans-serif; font-size:12; border: 1px solid #cccccc;">
					<?
					$selFavData = db_query("SELECT * FROM boomerange_inventory_gaylords_favorite WHERE user_id = '".$user_id."' && company_id = ".$company_id." and fav_status = 1 ORDER BY id DESC", db());
								...
								if($rowBoxDt['b2b_id'] > 0){
									...
									if($rowInvDt["loops_id"] > 0){
										...
										<tr class="<?=$rowclr?>">
											...
											<td class=''>
												<? if ($rowsFavData['fav_status'] == 1) { ?><input type="button" name="favItemIds" id="favItemIds<?=$rowsFavData['id'];?>" value="Remove" onclick="favitem_remove(<?=$rowsFavData['id'];?>, <?=$company_id;?>, <?php echo $user_id; ?>)"> <? }?>
											</td>	
					...
					<tr>
						<td colspan="18">
							...
							<input type="hidden" name="hdnCompanyId" value="<?=$company_id;?>">
							<input type="button" name="btnAddFavoriteInv" id="btnAddFavoriteInv" value="Add new favorite inventory" style="cursor:pointer;" onclick="add_inventory_to_favorite(<?php echo $company_id .','.$user_id;?>)" >
						</td>
					</tr>
				</table>
				</form>
		...
		<td colspan="6" >
				<form name="frmhideItems" method="post" action="boomerang_users_profile.php?ID=<?=$_REQUEST['ID'];?>">
				<table width="100%" cellspacing="1" cellpadding="1"border="0" style="font-family:Arial, Helvetica, sans-serif; font-size:12; border: 1px solid #cccccc;">
					<?
					db();
					$select_user_companies = db_query("SELECT company_id FROM boomerang_user_companies where user_id = '" . $user_id . "'");
					...
					if (tep_db_num_rows($select_user_companies) > 0) {
						while ($row1 = array_shift($select_user_companies)) {
							...
							$dt_view_res = array_shift($company_data_qry);
							$company_id = $dt_view_res['ID'];
							echo '<tr align="center"><td colspan="19" width="320px" align="left" bgcolor="#C1C1C1">'.$dt_view_res['company'].'</td></tr>';
				
					$selhideData = db_query("SELECT * FROM boomerang_inventory_hide_items WHERE company_id = ".$company_id." and hideItems = 1 and user_id =".$user_id." ORDER BY id DESC", db());
								...
								if($rowBoxDt['b2b_id'] > 0){
									...
									if($rowInvDt["loops_id"] > 0){
										...
										<tr bgcolor="<?=$rowclr?>" >
											...
											<td class=''>
												<? if ($rowsFavData['hideItems'] == 1) { ?><input type="button" name="hideItemIds" id="hideItemIds<?=$rowsFavData['id'];?>" value="Remove" onclick="hideitem_remove(<?=$rowsFavData['id'];?>, <?=$_REQUEST['ID'];?>, <?php echo $user_id ?>)"> <? }?>
											</td>	
					...
					<tr>
						<td colspan="18">
							...
							<input type="button" name="btnAddHideInv" id="btnAddHideInv" value="Add New Inventory to Hide" style="cursor:pointer;" onclick="add_inventory_to_hide(<?php echo $company_id; ?>, <?php echo $user_id;?>)" >
						</td>
					</tr>
					<?php 
...
<script>
		...
		function add_inventory_to_favorite(compId, user_id){ 
		...
		xmlhttp.open("GET","boomerang_getBoxData.php?ID="+compId+"&&user_id="+user_id,true);			
		xmlhttp.send();/**/
	}

	function Remove_boxes_warehouse_data(compId, favB2bId, user_id, closeloop = 0) { 
		if (window.XMLHttpRequest) {
		  xmlhttp=new XMLHttpRequest();
		} else {
		  xmlhttp=new ActiveXObject("Microsoft.XMLHTTP");
		...
		if(closeloop == 1){
			xmlhttp.open("GET","boomerang_update_closeloop_inventory_data.php?compId="+compId+"&favB2bId="+favB2bId+"&user_id="+user_id+"&upd_action=2",true);			
			xmlhttp.send();
		}else{
			xmlhttp.open("GET","boomerang_update_favorite_inventory_data.php?compId="+compId+"&favB2bId="+favB2bId+"&user_id="+user_id+"&upd_action=2",true);			
			xmlhttp.send();	
		}
	}
	
	function Add_boxes_warehouse_data(compId, favB2bId, user_id, closeloop = 0) { 
		...
		if(closeloop == 1){
			xmlhttp.open("GET","boomerang_update_closeloop_inventory_data.php?compId="+compId+"&favB2bId="+favB2bId+"&user_id="+user_id+"&upd_action=1",true);			
			xmlhttp.send();
		}else{
			xmlhttp.open("GET","boomerang_update_favorite_inventory_data.php?compId="+compId+"&favB2bId="+favB2bId+"&user_id="+user_id+"&upd_action=1",true);			
			xmlhttp.send();
		}
	...
	function favitem_remove(favitemid, companyid,user_id) {
		document.location = "boomerang_users_profile.php?ID=" + companyid + "&favitemid=" + favitemid +"&user_id=" + user_id + "&favremoveflg=yes";
	}

	function add_inventory_to_hide(compId, user_id){ 
		...
		xmlhttp.open("GET","boomerang_getBoxDataHide.php?ID="+compId+"&user_id="+user_id,true);			
		xmlhttp.send();
	}
	function Remove_boxes_hide(compId, favB2bId, user_id) { 
		if (window.XMLHttpRequest) {
		  xmlhttp=new XMLHttpRequest();
		} else {
		  xmlhttp=new ActiveXObject("Microsoft.XMLHTTP");
		...
		xmlhttp.open("GET","boomerang_update_hide_inventory_data.php?compId="+compId+"&favB2bId="+favB2bId+"&user_id="+user_id+"&upd_action=2",true);			
		xmlhttp.send();	
	}
	
	function Add_boxes_hide(compId, favB2bId, user_id) { 
		...
		xmlhttp.open("GET","boomerang_update_hide_inventory_data.php?compId="+compId+"&favB2bId="+favB2bId+"&user_id="+user_id+"&upd_action=1",true);			
		xmlhttp.send();
		
	}

	function hideitem_remove(hideitemid, companyid, user_id) {
		document.location = "boomerang_users_profile.php?ID=" + companyid + "&hideitemid=" + hideitemid + "&user_id="+user_id+"&hideremoveflg=yes";
	}
