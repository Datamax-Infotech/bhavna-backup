...
$log_notes = preg_replace("/'/", "\'", $_POST["log_notes"]);
...
$tot_value = 0;
	...
	if(($_REQUEST["mark_no_inv"]=="yes")){
		...
		invoice_date = '" . $_POST["invoice_date"] . "', vendor_id = '" . $_POST["vendor_id"] . "', log_notes ='".$log_notes."' WHERE id = " . $trans_rec_id;
		$result_sort = db_query($sql_upd,db() );

		$insert_log_notes = db_query("INSERT INTO water_transaction_log_notes (`date`,`trans_id`,`employee_id`,`message`) VALUES('" . date("Y-m-d H:i:s") . "', '" . $trans_rec_id . "', '" . $_COOKIE['b2b_id'] . "', '".$log_notes."')");
...
if ($_POST["update"] == "yes") {  
	...
	$query_mtd = "SELECT poundpergallon_value, weight_unit, water_boxes_report_data.id, Estimatedweight , Estimatedweight_value, water_boxes_report_data.unit_count, 
	...
	if ($water_cron_flg_year < (date("Y")-1)){
		...
		$final_tot = str_replace(",", "" , $final_tot);
