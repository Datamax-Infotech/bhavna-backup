...
<head>
	...
	<!-- 
	<div class="modal_line_items_table p-5">
		<div class=" contentEditable  line-item-text-area" contenteditable="true" style="height:200px">Hello WOrld</div>
	</div>
	-->
<body>
	...
	?>
		...
		<div id="custom-menu" class="context-menu">
			...
			<ul>
				...
				<li><span class="d-flex align-items-center"><input type="checkbox" id="group-line-item" class="mr-2">Mark as Section Header</span></li>
				<li><span id="setting_modal_btn" data-toggle="modal" data-target="#settingModal"><i class="fa fa-cog mr-2"></i>Setting </span></li>
			</ul>
		</div>
															...
															<tr class="<?php echo $mapped_data['line_item'] == 1 ? "list-item-selected" : "list-item-not-selected" ?>" id="list-item-selected-<?php echo $table_no.'-'.$row_no;?>" table_no="<?php echo $table_no; ?>" row_no="<?php echo $row_no; ?>">
																<td>
																	<div class="d-flex">
																		...
																		<small><i class="marked_as_custom_header fa fa-star text-danger mr-1 <?php echo $mapped_data['custom_header'] == 1 ? "d-block" : 'd-none'?>"></i></small>
																		<?php 
																			$custom_head_val = "";
																...
																?>
																	...
																	<div contenteditable="true" class="contentEditable line-item-text-area <?php echo $mapped_data['line_item'] == 1 ? '' : 'd-none';?>" onmouseup="updateCursorPosition(this,<?php echo $table_no; ?>,<?php echo trim($coordinates_arr[0]) . ',' . trim($coordinates_arr[1]) . ',' . trim($coordinates_arr[2]) . ',' . trim($coordinates_arr[3]) . ',' . trim($pageNo);?>)" oninput="set_value_of_item(this)"><?php echo trim($field_data_arr[$index1]); ?></div>
				...
				<div class="container-fluid p-0">
					...
					<div id="contentsplitter" style="height:100vh">
						...
						<button type="submit" id="choose-ocr" class="btn btn-outline-primary btn-sm">Process OCR</button>
								...
								$mainstr = "CustomerAddress^[object Object]^0.88|
								0.8916^2.6354x
								3.1039^2.6254x
								3.1053^2.95x
								0.893^2.96x
								1
								|Keypair|<br>
								CustomerAddressRecipient^Mojave Foods
								UCB Zero Waste, LLC.^0.885|
								0.8787^2.2201x
								2.5882^2.2201x
								2.5882^2.5829x
								0.8787^2.5829x
								...
								1
								|Keypair|<br>
								DueDate^Mon Oct 16 2023 19:00:00 GMT-0500 (Central Daylight Time)^0.942|
								5.9166^2.8025x
								6.4419^2.8073x
								6.4419^2.9648x
								5.9118^2.9696x
								...
								1
								|Keypair|<br>
								Weight Adjusted :^100.0 Pallets^0.322|
								4.2118^3.8913x
								4.8573^3.8958x
								4.8565^4.0149x
								4.211^4.0104x
								1
								|Keypair|<br>
								Weight Adjusted :^600.0 Pallets^0.676|
								4.207^4.1584x
								4.866^4.1584x
								4.866^4.2778x
								4.207^4.2778x
								...
								1
								|Keypair|<br>
								Check / Ref^undefined^0.705|
								|Keypair|<br>
								Prepared By^Gil Dodson^0.884|
								2.0203^10.2089x
								2.5873^10.2267x
								2.5831^10.3588x
								2.0162^10.341x
								...
								<td>WT Ticket #S 386942^undefined|
								1.2038^3.405x
								2.6032^3.405x
								2.6032^3.6115x
								1.2038^3.6115x
								...
								<td>Release # / Deliv ANA^undefined|
								1.2038^4.2946x
								3.8197^4.2867x
								3.8197^4.5568x
								1.2038^4.5647x
								...
								<table border=1>
								<tr>
								<td>Payment Information^undefined|
								4.3742^9.2287x
								5.9548^9.224x
								5.9548^9.3863x
								4.3742^9.3911x
	...
	if ($mainstr != "") {
						...
						if (index > 0) {
							...
							<tr class="list-item-not-selected" id="list-item-selected-${count}-${uniq_count2}" table_no="${count}" row_no="${uniq_count2}">
								<td>
									...
									<small><i class="marked_as_custom_header fa fa-star text-danger mr-1 d-none"></i></small>
		...
		<script>
			...
			$(document).ready(function() {
				...
				//$('.mapping_table tr.list-item-selected').on('contextmenu', function(e) {
				$('.mapping_table').on('contextmenu', 'tr.list-item-selected', function(e) {
					...
					// Stop the document click event from hiding the menu immediately
					return false;
				});
				...
				$('#group-line-item').change(function() {
					...
					if ($(this).prop('checked')) {
						...
						$('#list-item-selected-' + table_no + '-' + row_no).find('.line-item-text-area').addClass('font-weight-bold');
					} else {
						$('#group_text_check_' + table_no + "_" + row_no).val(0);
						$('#list-item-selected-' + table_no + '-' + row_no).find('.line-item-text-area').removeClass('font-weight-bold');
					}
				})
			...
			function split_cols_value() {
				...
				selected_cells.each(function() {
					...
					if (separatorVal === "" && isNaN(numberChars) && numberChars < 1 && splitString === "") {
						alert("Please Select Text OR enter Separator OR No of Character to split.");
						return;
					}
					if (separatorVal != "" && !isNaN(numberChars) && numberChars > 1 && splitString !== "") {
						alert("Please Do Either of Select Text OR Enter Separator OR No of Character to split.");
						return;
					}
					var beforeChars = "";
					var afterChars = "";
					if (splitString != "") {
						var spaceIndex;
						if (direction === "LTR") {
							spaceIndex = current_cell_val.indexOf(splitString);
						} else { // Assuming RTL
							spaceIndex = current_cell_val.lastIndexOf(splitString);
						}
						...
						if (direction === "LTR") {
							spaceIndex = current_cell_val.indexOf(separatorVal);
						} else { // Assuming RTL
							spaceIndex = current_cell_val.lastIndexOf(separatorVal);
						}
					...
					var conf = confirm(`Splited Cell value will be " ${beforeChars} " & " ${afterChars} "`);
					if (conf) {
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						...
						<div class="text-center">
							<button onclick="merge_cells()" class="btn btn-primary btn-sm">Merge Cells</button>
							<button onclick="split_cells()" class="btn btn-primary btn-sm">Split Cell</button>
							<!-- <button onclick="shift_cells()" class="btn btn-primary btn-sm">Shift Cell</button> -->
						</div>
							...
							<div class="row align-items-end justify-content-center">
								<div class="col-md-6">
									<div class="card py-3">
										...
										<div class="col-md-12">
											<p class="text-danger mb-1">Do either of Select Text OR Enter Separator OR Number of Charactor to Split data! </p>
										</div>
										<div class="form-group col-md-12">
											<label>Select Text to Split</label>
											<div contenteditable="true" id="select_text_div" class="contentEditable"></div>
										</div>
										<div class="form-group col-md-12">
											<label>Separator</label>
											<input type="text" id="separator" class="form-control form-control-sm" />
										</div>
										<div class="form-group col-md-12">
											<label>Number of Characters:</label>
											<input type="number" id="numChars" class="form-control form-control-sm" />
										</div>
									</div>
									<div class="text-center">
										<button onclick="split_cols_value()" class="btn btn-primary btn-sm mt-2" style="height: max-content">Split</button>
										<button onclick="cancel_split_cols_value()" class="btn btn-primary btn-sm mt-2" style="height: max-content">Cancel</button>
									</div>
								</div>
							</div>
						</div>
