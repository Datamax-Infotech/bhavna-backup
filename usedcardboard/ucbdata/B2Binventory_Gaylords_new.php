					...
					<div class="row">
						...
						<form id="filter_form" class="w-100">
							...
							?>
								...
								<input type="hidden" class="w-100" name="txtaddress" id="txtaddress" value="<? echo $shipAddress; ?>" />
								<input type="hidden" class="w-100" name="txtaddress2" id="txtaddress2" value="<? echo $shipAddress2; ?>" />
								<input type="hidden" class="w-100" name="txtcity" id="txtcity" value="<? echo $shipCity; ?>" />
								<input type="hidden" class="w-100" name="txtstate" id="txtstate" value="<? echo $shipState; ?>" />
								<input type="hidden" class="w-100" name="txtzipcode" id="txtzipcode" value="<? echo $shipZip; ?>" />
								...
								<? if ($rec_found == "yes") { ?>
									...
									<? } else { ?>
										...
										<div class="text-center">
											...
											<button type="button" class="btn apply-filter btn-sm" id="apply-deliverydata" onclick="copy_add_from_cust_to_main(); show_inventories('', 1, 1); functionLoaded = false; show_inventories('2');">Apply Address</button>
				...
				<div class="col-md-10">
					...
					<div class="row justify-content-end align-items-end m-0" style="width:75%">
						...
						<div class="col-md-1 p-0 ml-1 text-center">
							...
							<button class="btn control_view_btn btn-active-view btn-sm" id="table-view-button" onclick="change_view_type('table_view')"><i class="fa fa-align-justify"></i></button>
							...
							<button class="btn control_view_btn btn-sm" id="grid-view-button" onclick="change_view_type('grid_view')"><i class="fa fa-th-large"></i></button>
						</div>
					</div>
						...
						if (!$.isArray(result) || result.length == 0) {
							...
							if (view_type == "grid_view") {
								...
								$.each(result, function(index, res) {
									...
									if (shown_in_client_flg == 1) {
										if (document.getElementById("address_book")) {
											...
											var address_details = "enter_add1=" + enter_add1 + "&enter_add2=" + enter_add2 + "&enter_txtcity=" + enter_txtcity + "&enter_txtstate=" + enter_txtstate + "&enter_txtzipcode=" + enter_txtzipcode + "&enter_add_id=" + enter_add_id;
										}
									}
									...
									result_str += '<div class="col-lg-3 col-md-4 col-sm-6 mt-4 product-box-grid">';
									...
									var system_description = res['boxheight'] + " " + res['boxwall'] + " " + res['box_desc_bottom'];

									result_str += '<div class="product-description-grid">';
									result_str += '<h6 class="sysdescription"> ' + system_description + ' </h6>';
									result_str += '<h4 class="mb-0"><b>' + res["price"] + '</b></h4>';
									if ($('#txtzipcode').val() == "") {
										result_str += '<p class="highlight-detail-grid my-1" onclick="get_miles_away(); return false;">Add zip for miles away</p>';
									} else {
										result_str += '<p class="highlight-detail-grid my-1">' + res["distance"] + '</p>';
									}
									...
									result_str += '<div id="cal_delh4' + res['b2b_id'] + show_urgent_box + '"><a class="text-primary" href="#" onclick="calculate_delivery(' + delivery_cal_para + ', ' + show_urgent_box + '); return false"><u>Calculate Delivery (FTL)</u></a></div>';
									result_str += '</div>';

									result_str += '</div>';

									result_str += '<div class="product-additional-info px-3">';
									result_str += '<p class="quantity_available_now">Quantity Available Now</p>';
									result_str += '<p class="load_ship">' + res["colorvalueQty"] + '</p>';

									result_str += '<div class="">';
									result_str += `<p><span>ID = ${res["b2b_id"]}</span><span class="float-right"> Lead Time = <span style="${res["td_leadtime_bg_color"]}"> ${res["box_lead_time"]} </span></span></p>`;
									result_str += `<p><span>% of FTL = <span style="${td_bg_color}">${perofftl}</span></span><span class="float-right"> FTL = ${res["ftl_qty"].toLocaleString('en-US')} </span></p>`;
									result_str += '</div>';

									result_str += '</div>';
									var favorite = res['favorite'];
									result_str += `<div class="view_item_grid"><span class="fav_icon" onclick="change_fav_status(this, ${res['companyID']},${res['b2b_id']},${favorite},<?php echo $user_id; ?>)">`;
									result_str += favorite == 1 ? '<i class="fa fa-heart fav-added"></i>' : '<i class="fa fa-heart fav-removed"></i>';
									result_str += '</span>';
									result_str += '<a target="_blank" href="https://b2b.usedcardboardboxes.com/index_new.php?id=' + res['loop_id_encrypt_str'] + '&uid=<?php echo urlencode(encrypt_password($user_id)); ?>&${address_details}" class="btn btn-cart-grid">VIEW ITEM</a>';
									result_str += '</div>';
									result_str += '</div>';
									result_str += '</div>';
								});
								...
								if (show_urgent_box == 1) {
									...
									result_str_urgent += '<div class="dashboard_heading" style="font-size: 16px;font-family: Titillium Web, sans-serif;font-weight:400;">For a limited time, these gaylords are on sale!</div>';
									result_str_urgent += '<div class="container-fluid mb-4">';
									result_str_urgent += '<div class="px-2">';
									result_str_urgent += '<div class="row mx-auto">';
									result_str_urgent += '<div id="gallery" class="carousel slide w-100 align-self-center data-ride="carousel">';
									result_str_urgent += '<ol class="carousel-indicators">';
									for (var ind = 0; ind < result.length; ind++) {
										var active_cls = ind == 0 ? 'class="active"' : '';
										result_str_urgent += '<li data-target="#gallery" data-slide-to="' + ind + '" ' + active_cls + '></li>';
									}
									result_str_urgent += ' </ol>';
									result_str_urgent += '<div class="carousel-inner mx-auto" role="listbox" data-toggle="modal" data-target="#lightbox">';
									$.each(result, function(index, res) {
										var td_bg_color = "";
										if (res["td_bg"] == "green") {
											td_bg_color = " color: green;";
										...
										result_str_urgent += '<div class="col-lg-3 col-md-4 col-sm-6 mt-4 product-box-grid">';
										result_str_urgent += '<div class="product-bg-light">';
										var prod_src = res['img'] == "" ? default_img : "../boxpics_thumbnail/" + res['img'];
										result_str_urgent += '<a target="_blank" href="https://b2b.usedcardboardboxes.com/?id=' + res['loop_id_encrypt_str'] + '"><img src="' + prod_src + '" class="img-fluid product-img"/></a>';

										var system_description = res['boxheight'] + " " + res['boxwall'] + " " + res['box_desc_bottom'];

										result_str_urgent += '<div class="product-description-grid">';
										result_str_urgent += '<h6 class="sysdescription"> ' + system_description + ' </h6>';
										result_str_urgent += '<h4 class="mb-0"><b>' + res["price"] + '</b></h4>';
										if ($('#txtzipcode').val() == "") {
											result_str_urgent += '<p class="highlight-detail-grid my-1" onclick="get_miles_away(); return false;">Add zip for miles away</p>';
										} else {
											result_str_urgent += '<p class="highlight-detail-grid my-1">' + res["distance"] + '</p>';
										}
										...
										//result_str_urgent += '</div>';
										var favorite = res['favorite'];
										result_str_urgent += `<div class="view_item_grid"><span class="fav_icon" onclick="change_fav_status(this, ${res['companyID']},${res['b2b_id']},${favorite},<?php echo $user_id; ?>)">`;
										result_str_urgent += favorite == 1 ? '<i class="fa fa-heart fav-added"></i>' : '<i class="fa fa-heart fav-removed"></i>';
										result_str_urgent += '</span>';
										result_str_urgent += '<a target="_blank" href="https://b2b.usedcardboardboxes.com/index_new.php?id=' + res['loop_id_encrypt_str'] + '&uid=<?php echo urlencode(encrypt_password($user_id)); ?>&${address_details}" class="btn btn-cart-grid">VIEW ITEM</a>';
										result_str_urgent += '</div>';
										result_str_urgent += '</div>';
										result_str_urgent += '</div>';
										result_str_urgent += '</div>';
									...
									result_str_urgent += ' <a class="carousel-control-prev w-auto" href="#gallery" role="button" data-slide="prev">';
									result_str_urgent += ' <span class="carousel-control-prev-icon" aria-hidden="true"></span><span class="sr-only">Previous</span>';
									result_str_urgent += ' </a>';
									result_str_urgent += '<a class="carousel-control-next w-auto" href="#gallery" role="button" data-slide="next">';
									result_str_urgent += ' <span class="carousel-control-next-icon" aria-hidden="true"></span><span class="sr-only">Next</span>';
									result_str_urgent += ' </a>';
									result_str_urgent += '</div>';
									result_str_urgent += '</div>';
									result_str_urgent += '</div>';
								...
								var shown_in_client_user_id_hide = <?php echo json_encode($user_id_hide_price); ?>;
								...
								if (show_urgent_box == 1) {
									...
									$.each(result, function(index, res) {
										...
										sub_table_frequency_ftl += parseFloat(res['frequency_ftl']);
										result_str_urgent += '<div class="carousel-item">';
										result_str_urgent += '<div class="col-lg-2 col-md-4 col-sm-6 mt-4 product-box-grid">';
										result_str_urgent += '<div class="product-bg-light">';
										var prod_src = res['img'] == "" ? default_img : "../boxpics_thumbnail/" + res['img'];
										result_str_urgent += '<a target="_blank" href="https://b2b.usedcardboardboxes.com/?id=' + res['loop_id_encrypt_str'] + '"><img src="' + prod_src + '" class="img-fluid product-img"/></a>';
