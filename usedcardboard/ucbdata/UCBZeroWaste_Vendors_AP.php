...
<head>
	...
	<script>
		...
		function displarepsorteddata(comp_id, vendor_id, dmadepayment, dtfrom, dtto, receives, payables, columnno, sortflg) {
			...
			//console.log( "UCBZeroWaste_Vendors_AP_child.php?comp_sel=" + comp_id + "&vendors_dd=" + vendor_id + "&ddMadePayment=" + dmadepayment + "&date_from=" + dtfrom + "&date_to=" + dtto + "&receivables=" + receives + "&payables=" + payables + "&columnno=" + columnno + "&sortflg=" + sortflg +"&page="+'<?php echo $page_no; ?>');
			xmlhttp.open("GET", "UCBZeroWaste_Vendors_AP_child.php?comp_sel=" + comp_id + "&vendors_dd=" + vendor_id + "&ddMadePayment=" + dmadepayment + "&date_from=" + dtfrom + "&date_to=" + dtto + "&receivables=" + receives + "&payables=" + payables + "&columnno=" + columnno + "&sortflg=" + sortflg +"&page=1", true);
			xmlhttp.send();
		}
		function update_page_data(element, comp_id, vendor_id, dmadepayment, dtfrom, dtto, receives, payables){
			...
			xmlhttp.open("GET", "UCBZeroWaste_Vendors_AP_child.php?comp_sel=" + comp_id + "&vendors_dd=" + vendor_id + "&ddMadePayment=" + dmadepayment + "&date_from=" + dtfrom + "&date_to=" + dtto + "&receivables=" + receives + "&payables=" + payables + "&columnno=" + sorting_column + "&sortflg=" + sorting_flg +"&page="+ page, true);
			xmlhttp.send();
		}
		...
		function send_invoice(element, transid, flg){
			$.ajax({
				...
				method: 'get',
				data: {transid, flg, 'send_invoice':1},
				async: false,
				beforeSend: function() {
		...
		where make_receive_payment = 1 and vendor_id='" . $termHealth["vendor_id"] . "' " . $whrMadePayConditn . " " . $whr_flt_inv_due_dt . " $swhere_condition group by Status, company_id, water_transaction.id having sum(amount) <= 0 order by made_payment desc";
			...
			<?php
				...
				if ($pagination_string != "") {
					...
					if (is_array($queryParams) && array_key_exists('page', $queryParams)) {
					...
					if ($current_page > 1) {
						echo '<a href="' . $baseURL . '&page=' . ($current_page - 1) . '">&laquo; Previous</a>';
					}
					for ($page = 1; $page <= $total_pages; $page++) {
						echo '<a class="' . ($page == $current_page ? 'active_page' : '') . '" href="' . $baseURL . '&page=' . $page . '">' . $page . '</a>';
					}
					if ($current_page < $total_pages) {
						echo '<a href="' . $baseURL . '&page=' . ($current_page + 1) . '">Next &raquo;</a>';
					}
					*/
					...
					echo '<div><font color="red">"END OF REPORT"</font> </div>';
				}
