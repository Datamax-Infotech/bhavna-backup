...
$user_id = isset($_REQUEST['loginid']) && $_REQUEST['loginid']!="" ? $_REQUEST['loginid'] : 0;
                ...
                <div class="col-md-12 filter-box1">
                    <div class="row">
						...
						<?php if(isset($_REQUEST['loginid']) && $_REQUEST['loginid'] != ""){ 
							...
							if(tep_db_num_rows($user_address_qry) > 0){
							?>
								...
								<select name="address_book" id="address_book" onchange="change_addressbook()">
									<?php
										while($user_address = array_shift($user_address_qry)){
											$address = $user_address['company'].", ".$user_address['addressline1'].", ".$user_address['city'].", ".$user_address['state'].", ".$user_address['zip'];
											$address = substr($address, 0, 60);
											$address = $address."...";
											echo "<option value='".$user_address['id']."'>".$address."</option>";
										}
									?>
								</select>
							</div>
				...
				if( !$.isArray(result) ||  result.length ==0 ) {
					...
					$.each(result,function(index,res){
						...
						result_str += `	<td style="background-color:${row_alternate_color}" bgColor="${res['td_bg']}">${res["distance"]}</td>
										...
										if (shown_in_client_flg == 1) {
						result_str += `		<td style="background-color:${row_alternate_color}" class="text-center" bgColor="${res['td_bg']}"><a class="text-primary" href="https://b2b.usedcardboardboxes.com/index_new.php?id=${res['loop_id_encrypt_str']}" target="_blank"><u>View Item</u></a></td>`;
										}else{
