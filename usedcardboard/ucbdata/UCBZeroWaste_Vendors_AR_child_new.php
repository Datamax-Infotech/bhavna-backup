	...
	<tr class="display_maintitle">
		...
		if (isset($_REQUEST["vendors_dd"])) {
			if ($_REQUEST["vendors_dd"] != "All" && $_REQUEST["vendors_dd"] != "") {
				$vendorsQry = "Select *, water_vendors.id as vid from water_transaction inner join water_vendors on water_transaction.vendor_id=water_vendors.id inner join water_vendors_payable_contact on water_transaction.vendor_id=water_vendors_payable_contact.water_vendor_id where make_receive_payment = 1 and vendor_id = '".$_REQUEST["vendors_dd"]."' group by vendor_id";
				//$Vwhere_condition = " and vendor_id = " . $_REQUEST["vendors_dd"] ;
			
			}else{
				$vendorsQry = "
				Select *, water_vendors.id as vid from water_transaction inner join water_vendors on water_transaction.vendor_id=water_vendors.id inner join water_vendors_payable_contact on water_transaction.vendor_id=water_vendors_payable_contact.water_vendor_id where make_receive_payment = 1 group by vendor_id";
				
			}
		}else{
			$vendorsQry = "
			Select *, water_vendors.id as vid from water_transaction inner join water_vendors on water_transaction.vendor_id=water_vendors.id inner join water_vendors_payable_contact on water_transaction.vendor_id=water_vendors_payable_contact.water_vendor_id 
			where make_receive_payment = 1 group by vendor_id";
		}
		...
		if (isset($_REQUEST["columnno"])) {
			...
			if ($_REQUEST["columnno"] == "2" && $_REQUEST["sortflg"] == "2") {
				...
				$vendorsQry_order = " order by loop_warehouse.company_name desc";
			}	
			...
			if ($_REQUEST["columnno"] == "11" && $_REQUEST["sortflg"] == "1") {
				$vendorsQry .= " order by payable_payment_terms_accepted_by_ucbzerowaste asc";
				$vendorsQry_order = " order by payable_payment_terms_accepted_by_ucbzerowaste asc";
			}	
			if ($_REQUEST["columnno"] == "11" && $_REQUEST["sortflg"] == "2") {
				$vendorsQry .= " order by payable_payment_terms_accepted_by_ucbzerowaste desc";
				$vendorsQry_order = " order by payable_payment_terms_accepted_by_ucbzerowaste desc";
			}	
		...
		//echo $vendorsQry_order;
		$whr_flt_inv_due_dt = ""; $flt_inv_due_dt = "";
		...
		//while ($data_row = array_shift($v_res)) {
			...
			if (isset($_REQUEST["vendors_dd"])) {
				if ($_REQUEST["vendors_dd"] != "All" && $_REQUEST["vendors_dd"] != "") {
					$vendorQry = "Select *, water_transaction.id as transid, loop_warehouse.id as warehouse_id,water_vendors_payable_contact.id as receivable_id, sum(amount) as amt, loop_warehouse.company_name, loop_warehouse.b2bid, water_transaction.id as transid 
					...
					where make_receive_payment = 1 and vendor_id='". $_REQUEST["vendors_dd"] ."' ". $whrMadePayConditn ." " . $whr_flt_inv_due_dt . " $swhere_condition group by company_id, 
